...
impl WasmRpcServer {
    ...
    pub async fn new(rpc_config_json: &str) -> Result<WasmRpcServer, JsError> {
        let rpc_config: RpcConfig = serde_json::from_str(rpc_config_json).map_err(|e| JsError::new(&format!("Parse RPC config error: {}", e)))?;
    ...
    pub async fn exec_contract_call_json(&mut self, pk_hash: &str, contract_calls_json: &str) -> Result<String, JsError> {
        let contract_call_args: Vec<ContractCallArgs> =
            serde_json::from_str(contract_calls_json).map_err(|e| JsError::new(&format!("Parse exec calls JSON error: {}", e)))?;
    ...
    #[wasm_bindgen]
    pub async fn register_user_with_type(&mut self, private_key: &str, sign_type: &str, fingerprint: Option<String>) -> Result<String, JsError> {
        let private_key = QHashOut::<F>::from_str(private_key).map_err(|e| JsError::new(&format!("Parse private key error: {}", e)))?;
        let sign_type = SignType::from_str(sign_type, true).map_err(|e| JsError::new(&format!("Parse sign type error: {}", e)))?;
    ...
    #[wasm_bindgen]
    pub async fn add_user_with_type(&mut self, private_key_str: &str, sign_type: &str, fingerprint: Option<String>) -> Result<String, JsError> {
        let private_key = QHashOut::<F>::from_str(private_key_str).map_err(|e| JsError::new(&format!("Parse private key error: {}", e)))?;
