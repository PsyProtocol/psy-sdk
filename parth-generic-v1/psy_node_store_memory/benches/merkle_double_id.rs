...
async fn setup_memory_store<Hash: QDBHashBase, Hasher: MerkleZeroHasher<Hash> + Send + Sync>(keyspace: String) -> anyhow::Result<(InMemoryCoreStore<Hash, Hasher>, InMemoryTableIdentifier)> {
    let store = InMemoryCoreStore::<Hash, Hasher>::new(
    );




    Ok((store, InMemoryTableIdentifier::new_with_keyspace(&keyspace, "basic_merkle_nodes_double_id")))
}
...
fn bench_merkle_double_id_internal<Hash: BenchFastRand + QDBHashBase, Hasher: MerkleZeroHasher<Hash> + Send + Sync>(c: &mut Criterion, node_count: usize) {
    ...
    let (store, double_id_merkle_table) = rt
        .block_on(setup_memory_store::<Hash, Hasher>(keyspace_prefix))
        .unwrap();
...
/* 
    ...
    group.bench_function(&format!("insert_{}_db_set_double_id_merkle_nodes_from_fast_serialized", node_count), |b| {
        b.iter(|| {
            rt.block_on(store.db_set_double_id_merkle_nodes_from_fast_serialized(&double_id_merkle_table, checkpoint_id_test_a, &&fast_serialized_double[QBLOB_TREE_NODE_BATCH_HEADER_SIZE..])).unwrap();
        });
    });
    group.bench_function(&format!("insert_{}_db_set_single_id_merkle_nodes_from_fast_serialized", node_count), |b| {
        b.iter(|| {
            rt.block_on(store.db_set_single_id_merkle_nodes_from_fast_serialized(&double_id_merkle_table, checkpoint_id_test_a, &fast_serialized_single[QBLOB_TREE_NODE_BATCH_HEADER_SIZE..])).unwrap();
        });
    });
    group.bench_function(&format!("insert_{}_db_set_zero_id_merkle_nodes_from_fast_serialized", node_count), |b| {
        b.iter(|| {
            rt.block_on(store.db_set_zero_id_merkle_nodes_from_fast_serialized(&double_id_merkle_table, checkpoint_id_test_a, &fast_serialized_zero[QBLOB_TREE_NODE_BATCH_HEADER_SIZE..])).unwrap();
        });
    });
}
