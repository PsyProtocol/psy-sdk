...
use parth_core::{crypto::hash::{merkle_proof::DeltaMerkleProofCore, traits::MerkleHasher}, pgoldilocks::{PoseidonHasher, QHashOut}};
...
impl<F: RichField> VariableHeightDeltaMerkleProofOpt<F> {
    ...
    fn compute_root_from_value(
        ...
        for (i, &bit) in index_bits.iter().enumerate() {
            ...
            current_hash = if bit {
                PoseidonHasher::two_to_one(sibling, &current_hash)
            } else {
                PoseidonHasher::two_to_one(&current_hash, sibling)
            };
            remaining_height -= 1;
        }
        current_hash
