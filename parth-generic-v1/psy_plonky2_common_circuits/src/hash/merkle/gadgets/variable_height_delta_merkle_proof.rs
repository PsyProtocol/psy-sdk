use psy_plonky2_basic_helpers::builder::{comparison::CircuitBuilderComparison, hash::core::CircuitBuilderHashCore, math::core::CircuitBuilderCoreMathHelpers, select::CircuitBuilderSelectHelpers};
use plonky2::{
...
impl VariableHeightDeltaMerkleProofGadget {
    ...
    >(
        ...
        for (ind, (&bit, &sibling)) in index_bits.iter().zip(siblings).enumerate() {
            ...
            let old_proposed_state = builder.hash_two_to_one::<H>(
                old_left,
                old_right,
            );
            ...
            let new_proposed_state = builder.hash_two_to_one::<H>(
                new_left,
                new_right,
            );
...
mod tests {
    ...
    impl TestVariableHeightDeltaMerkleProofCircuit {
        pub fn new(max_height: usize) -> Self {
            ...
            let variable_delta_merkle_proof_gadget = VariableHeightDeltaMerkleProofGadget::add_virtual_to_full::<PoseidonHash, F, D>(
                &mut builder,
                max_height,
                Some(height),
            );
