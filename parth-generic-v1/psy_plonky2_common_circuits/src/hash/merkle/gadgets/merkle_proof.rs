...
use psy_plonky2_basic_helpers::builder::optional_inputs::CircuitBuilderOptionalInputs;
...
[
  ...
  },
  {
    ...
    "#;
    type MerkleProof<F> = MerkleProofCore<QHashOut<F>>;
    ...
    fn create_mp_circuit_for_proof_a(mp: &MerkleProof<F>) {
        ...
        mp_gadget.set_witness_core_proof_q(&mut pw, mp).unwrap();
    ...
    fn create_mp_circuit_for_proof_b(mp: &MerkleProof<F>) {
        ...
        let index = builder.constant_u64(mp.index);
    ...
    fn merkle_proof_should_fail_a(mp: &MerkleProof<F>) {
        let config = CircuitConfig::standard_recursion_config();
        let mut builder = CircuitBuilder::<F, D>::new(config);
        let index = builder.constant_u64(mp.index + 1);
        let root = builder.constant_hash(mp.root.0);
        let mp_gadget = MerkleProofGadget::add_virtual_to_with_options::<PoseidonHash, F, D>(
            &mut builder,
            mp.siblings.len(),
