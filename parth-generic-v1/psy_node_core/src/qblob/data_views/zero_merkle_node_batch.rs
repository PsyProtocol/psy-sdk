...
impl QBlobZeroMerkleNodeBatchDataView {
    ...
    pub fn validate_zero_tree_nodes_batch_header_for_realm_context(
        ...
        header.is_valid_for_realm_context(chain_id, realm_id, realm_sub_id, unique_pending_id) && header.tree_type == tree_type
    }
    ...
    pub fn validate_zero_tree_nodes_batch_header_for_realm_context_get_clipped_ref(
        data: &[u8],
        chain_id: u32,
        realm_id: u64,
        realm_sub_id: u64,
...
impl QBlobZeroIdMerkleRecorder {
    pub fn new() -> Self {
        Self {
            map: HashMap::new(),
            blob: Vec::new(),
    ...
    pub fn new_with_size_hint(size_hint: usize) -> Self {
        Self {
            ...
            blob: Vec::with_capacity(size_hint * QMS_FAST_SERIALIZER_ZERO_ID_NODE_SIZE),
        }
    }
    pub fn new_with_header_and_size_hint(size_hint: usize) -> Self {
        let map = HashMap::with_capacity(size_hint);
        let mut blob = Vec::with_capacity(QBLOB_TREE_NODE_BATCH_HEADER_SIZE + size_hint * QMS_FAST_SERIALIZER_ZERO_ID_NODE_SIZE);
    ...
    pub fn record_and_compute_merkle_root<Hash: Q256BitHash, Hasher: MerkleHasher<Hash>>(
        ...
        let mut current_node = QMerkleStoreZeroIdNode {
    ...
    pub fn finalize_with_header(self, context: &QBlobWriterContextMetadataHeader, tree_type: QBlobMerkleNodeTreeType) -> Vec<u8> {
        ...
        let item_count = size_without_header as u64 / QMS_FAST_SERIALIZER_ZERO_ID_NODE_SIZE as u64;
        blob[0..QBLOB_TREE_NODE_BATCH_HEADER_SIZE].copy_from_slice(
            &QBlobZeroMerkleNodeBatchDataView::tree_header_from_context_and_counts(context, tree_type, item_count).to_bytes_fixed_size_array(),
        );
        blob
    }
}
...
mod tests {
    ...
    use parth_core::{
        crypto::hash::{
            merkle_proof::{compute_path_merkle_proof_generic, compute_root_merkle_proof_generic},
            traits::MerkleZeroHasher,
        },
        data::hash::{hash256::Hash256, merkle_node_key::SimpleMerkleNodeKey, merkle_store_key::QMerkleStoreZeroIdNode},
    ...
    use crate::qblob::{
        ...
        data_views::zero_merkle_node_batch::{QBlobZeroIdMerkleRecorder, QBlobZeroMerkleNodeBatchDataView},
        structs::common::blob_metadata_header::QBlobWriterContextMetadataHeader,
    };

    fn fuzz_merkle_hash_recorder_with_sibling_values_helper<
        Hash: QPGenRandom + Q256BitHash + std::fmt::Debug + Default,
    ...
    >(
        ...
        let mut recorder = if with_context.is_none() {
            QBlobZeroIdMerkleRecorder::new()
        } else {
            QBlobZeroIdMerkleRecorder::new_with_header_and_size_hint(count * height as usize)
        };
        ...
        let expected_values = final_key_set.iter().map(|key| tree.get_node_value(key)).collect::<Vec<Hash>>();

        let expected_nodes = final_key_set
            .iter()
            .zip(expected_values.iter())
            .map(|(key, value)| QMerkleStoreZeroIdNode {
        ...
        let got_nodes = if with_context.is_none() {
            QBlobZeroMerkleNodeBatchDataView::read_batch_zero_nodes_from_checked_payload::<Hash>(&blob)?
        } else {
            ...
            let (header, payload) = QBlobZeroMerkleNodeBatchDataView::validate_zero_tree_nodes_batch_header_for_realm_context_get_clipped_ref(
                &blob,
                chain_id,
                realm_id,
                realm_sub_id,
            ...
            assert_eq!(
                ...
                true,
                "Header is not valid for expected context"
            );
            QBlobZeroMerkleNodeBatchDataView::read_batch_zero_nodes_from_checked_payload::<Hash>(&payload)?
        };
    ...
    fn fuzz_merkle_hash_recorder_with_sibling_values_helper_proof<
        Hash: QPGenRandom + Q256BitHash + std::fmt::Debug + Default,
        Hasher: MerkleZeroHasher<Hash>,
    >(
        height: u8,
    ...
    fn test_merkle_hash_recorder_simple() -> anyhow::Result<()> {
        ...
        let mut recorder = QBlobZeroIdMerkleRecorder::new();
        ...
        let computed_recorded_root = recorder.record_and_compute_merkle_root::<Hash, Hasher>(index, height as u8, &value, &siblings);
        assert_eq!(expected_path.len(), height + 1, "Expected path length does not match height");
        assert_eq!(
            expected_path.last().unwrap(),
            &expected_root,
            "last expected path node does not match root"
        );
        ...
        let decoded_path_nodes = QBlobZeroMerkleNodeBatchDataView::read_batch_zero_nodes_from_checked_payload::<Hash>(&recorder.blob)?;
        ...
        let leaf_key = SimpleMerkleNodeKey { level: height as u8, index };

        let key_path_above = leaf_key.get_above_path_including_root();
        assert!(
            key_path_above.len() + 1 == expected_path.len(),
            "Key path length does not match expected path length"
        );
        ...
        assert_eq!(
            combined_keys.len(),
            expected_path.len(),
            "Combined keys length does not match expected path length"
        );

        let expected_zero_id_nodes = combined_keys
            .into_iter()
            .zip(expected_path.into_iter())
            .map(|(key, value)| QMerkleStoreZeroIdNode { key, value })
            .collect::<Vec<_>>();

        assert_eq!(
            decoded_path_nodes, expected_zero_id_nodes,
            "Decoded path nodes do not match expected nodes"
        );
    ...
    fn test_merkle_hash_recorder_multiple_proofs() -> anyhow::Result<()> {
        for i in 0..16 {
            fuzz_merkle_hash_recorder_with_sibling_values_helper::<Hash256, CoreSha256Hasher>(
                3,
                i,
                None,
        ...
        fuzz_merkle_hash_recorder_with_sibling_values_helper::<Hash256, CoreSha256Hasher>(3, 2, None, QBlobMerkleNodeTreeType::GlobalContractTree)
            .unwrap();
        fuzz_merkle_hash_recorder_with_sibling_values_helper::<Hash256, CoreSha256Hasher>(3, 2, None, QBlobMerkleNodeTreeType::GlobalUserTree)
            .unwrap();
        ...
        .unwrap();
        fuzz_merkle_hash_recorder_with_sibling_values_helper::<Hash256, CoreSha256Hasher>(64, 23, None, QBlobMerkleNodeTreeType::GlobalContractTree)
            .unwrap();
        fuzz_merkle_hash_recorder_with_sibling_values_helper::<Hash256, CoreSha256Hasher>(
            54,
            32,
        ...
        for i in 0..16 {
            fuzz_merkle_hash_recorder_with_sibling_values_helper::<PHash, PoseidonHasher>(3, i, None, QBlobMerkleNodeTreeType::GlobalContractTree)
                .unwrap();
        }
        fuzz_merkle_hash_recorder_with_sibling_values_helper_proof::<PHash, PoseidonHasher>(3, 255, None, QBlobMerkleNodeTreeType::GlobalContractTree)
            .unwrap();

        fuzz_merkle_hash_recorder_with_sibling_values_helper::<PHash, PoseidonHasher>(3, 2, None, QBlobMerkleNodeTreeType::GlobalContractTree).unwrap();
        fuzz_merkle_hash_recorder_with_sibling_values_helper::<PHash, PoseidonHasher>(3, 2, None, QBlobMerkleNodeTreeType::GlobalUserTree)
            .unwrap();

        fuzz_merkle_hash_recorder_with_sibling_values_helper::<PHash, PoseidonHasher>(24, 10, None, QBlobMerkleNodeTreeType::GlobalUserTree)
            .unwrap();
        fuzz_merkle_hash_recorder_with_sibling_values_helper::<PHash, PoseidonHasher>(32, 32, None, QBlobMerkleNodeTreeType::GlobalContractTree)
            .unwrap();
        fuzz_merkle_hash_recorder_with_sibling_values_helper::<PHash, PoseidonHasher>(64, 18, None, QBlobMerkleNodeTreeType::GlobalUserTree)
            .unwrap();
        fuzz_merkle_hash_recorder_with_sibling_values_helper::<PHash, PoseidonHasher>(54, 12, None, QBlobMerkleNodeTreeType::GlobalUserTree)
            .unwrap();
        fuzz_merkle_hash_recorder_with_sibling_values_helper::<PHash, PoseidonHasher>(16, 142, None, QBlobMerkleNodeTreeType::GlobalUserTree)
            .unwrap();
        ...
        for i in 0..16 {
            fuzz_merkle_hash_recorder_with_sibling_values_helper::<PHash, PoseidonHasher>(
                3,
                i,
                Some(context),
                QBlobMerkleNodeTreeType::GlobalContractTree,
    ...
    fn check_round_trip() -> anyhow::Result<()> {
        ...
        println!(
            "Serialization took: {:?}, ({}ms per node * {} nodes)",
            duration,
            duration.as_secs_f64() / (count as f64 * 1000f64),
            count
        );
        ...
        let duration = start_time.elapsed();
        println!(
            "Deserialization took: {:?}, ({}ms per node * {} nodes)",
            duration,
            duration.as_secs_f64() / (count as f64 * 1000f64),
            count
        );
    ...
    fn check_batches_unchecked() -> anyhow::Result<()> {
        ...
        let batch_nodes: Vec<Vec<QMerkleStoreZeroIdNode<Hash>>> = (0..number_of_batches)
            ...
            .collect();
        let batches = batch_nodes
            .iter()
            .map(|batch| QBlobZeroMerkleNodeBatchDataView::generate_zero_merkle_node_batch_blob_data_from_ref(context, tree_type, batch))
            .collect::<Vec<_>>();

        let start_time = std::time::Instant::now();
        let serialized_blob =
            QBlobZeroMerkleNodeBatchDataView::combine_zero_merkle_node_batch_blobs_unvalidated::<Hash>(batches, &context, tree_type)?;
        let duration = start_time.elapsed();
        println!(
            "Serialization took: {:?}, ({}ms per batch * {} batches)",
            duration,
            duration.as_secs_f64() / (number_of_batches as f64 * 1000f64),
            number_of_batches
        );
        ...
        let duration = start_time.elapsed();
        println!(
            "Deserialization took: {:?}, ({}ms per batch * {} batches)",
            duration,
            duration.as_secs_f64() / (number_of_batches as f64 * 1000f64),
            number_of_batches
        );
