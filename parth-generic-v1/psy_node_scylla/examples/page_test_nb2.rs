...
async fn main() -> Result<()> {
    ...
    let count = 500_000usize;//16777216usize;
    ...
    for i in 0..count {
        ...
        let _pp = tokio::task::spawn(async move {
            ...
            let node_index_i64 = u64_to_i64_exact(node.index);
                        ...
                        let res = session.execute_unpaged(&prepared, (level_i8, node_index_i64, checkpoint_q_id_i64)).await?;
                        let rows = res.into_rows_result()?;
                        if let Some(row) = rows.maybe_first_row::<(&[u8],)>()? {
                                return anyhow::Ok(Some(Hash256::from_slice_32bytes(row.0)?));
                        }
           anyhow::Ok(None)
        });
    }
    ...
    // Iterate through select result with paging
    let mut rows_stream = session
        .query_iter(format!("SELECT level, node_index, value FROM {}.{} WHERE level = ?", keyspace, table_name), (32i8,))
        .await?
        .rows_stream::<(i8, i64, Vec<u8>)>()?;
