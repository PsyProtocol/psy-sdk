...
use parth_core::{
    ...
    utils::{
        signed_helpers::{i64_to_u64_exact, u64_to_i64_exact},
        QPGenRandom,
    },
};
...
async fn main() -> Result<()> {
    ...
    let prepared = Arc::new(
        session
            .prepare(format!(
                "INSERT INTO {}.{} (level, bucket, node_index, checkpoint_id, value) VALUES (?, ?, ?, ?, ?)",
                keyspace, table_name
            ))
            .await?,
    );
    ...
    let select_1_statement = Statement::new(&format!(
        "SELECT value FROM {}.{} WHERE level = ? AND bucket = ? AND node_index = ? AND checkpoint_id <= ? LIMIT 1",
        keyspace, table_name
    ));
    ...
    debug_timer.event(format!("querying {count} rows with parallelism {parallelism}"));

    let checkpoint_q_id_i64 = convert_checkpoint_id_to_i64(1337 * 2);

    let query_results = Arc::new(DashMap::<SimpleMerkleNodeKey, Hash256>::new());
    ...
    for i in 0..count {
        ...
        let session = session.clone();
        let prepared = select_1_prepared.clone();
        let permit = sem.clone().acquire_owned().await;
        ...
        let _pp = tokio::task::spawn(async move {
            ...
            let node_index_i64 = u64_to_i64_exact(node.index);

            let res = session
                .execute_unpaged(&prepared, (level_i8, bucket, node_index_i64, checkpoint_q_id_i64))
                .await?;
            let rows = res.into_rows_result()?;
            if let Some(row) = rows.maybe_first_row::<(&[u8],)>()? {
                query_results.insert(node, Hash256::from_slice_32bytes(row.0)?);
            }
            anyhow::Ok(())
        });
    }
    ...
    let _results = query_results
        ...
        .collect::<Vec<_>>();
    debug_timer.event_batch_item_ref(format!("query_{count}_with_paralellism_{parallelism}"), "row", count);
    ...
    while let Some(next_row_res) = rows_stream.next().await {
        ...
        results.push(SimpleMerkleNode::new(
            i8_to_u8_exact(level_i8),
            i64_to_u64_exact(index_i64),
            Hash256::from_owned_32bytes(data.try_into().unwrap()),
        ));
    ...
    let mut rows_stream = session
        .query_iter(
            format!("SELECT level, node_index, checkpoint_id, value FROM {}.{}", keyspace, table_name),
            &[],
        )
        .await?
    ...
    while let Some(next_row_res) = rows_stream.next().await {
        ...
        if level_i8 != 32
            || g_checkpoint_id_i64 > max_checkpoint_id_i64
            || g_checkpoint_id_i64 < *seen_node_at_checkpoint_id_i64.get(&(level_i8, index_i64)).unwrap_or(&i64::MIN)
        {
            continue;
        }
    ...
    let mut rows_stream = session
        .query_iter(
            format!(
                "SELECT level, node_index, value FROM {}.{} WHERE level = ? ALLOW FILTERING",
                keyspace, table_name
            ),
            (32i8,),
        )
    ...
    loop {
        let (res, paging_state_response) = session.execute_single_page(&paged_prepared, &[], paging_state).await?;
    ...
    debug_timer.event_batch_item_ref(format!("fetched_all_pages_prepared_statement"), "row", total_rows_fetched);
