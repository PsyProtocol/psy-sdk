...
async fn main() -> Result<()> {
    ...
    for i in 0..count {
        ...
        tokio::task::spawn(async move {
            let level = ((i as u64)&0xffu64) as u8;
            let node_index = (i as u64)*3;
            let checkpoint_id = (i/100) as u64;
            let level_i8 = u8_to_i8_exact(level);
            let bucket = get_bucket_for_node(level, node_index);
            let node_index_i64 = u64_to_i64_exact(node_index);
            let checkpoint_id_i64 = convert_checkpoint_id_to_i64(checkpoint_id);
            let value = vec![level; 32];
            session
