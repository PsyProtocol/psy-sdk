...
impl<const QUEUE_TOPIC_ID: u32, QueueItem: PCoreQueueItemBase, Builder: QueueGathererItemBuilder + Send + Sync, Sub: QStandardEphemeralQueueSubscriber + Send + Sync>
        ...
        'gathering: loop {
            tokio::select! {
                ...
                maybe_msg =     stream.consume_ephemeral_queue_item_or_none_bytes(&queue_key, queue_key.realm_id, queue_key.realm_sub_id, queue_key.unique_id, queue_key.task_group as u32) => {
                    match maybe_msg {
                        Ok(Some(bytes)) => {
                            builder.update_from_queue_item(bytes).await?;
                        },
                        Ok(None) => {
                            // nop
