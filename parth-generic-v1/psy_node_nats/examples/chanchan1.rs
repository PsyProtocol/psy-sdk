...
pub trait QueueGathererItemBuilder: Sized {
    ...
    async fn create_new() -> anyhow::Result<Self>;
...
pub struct QueueKeyHelper<const QUEUE_TOPIC_ID: u32, QueueItem: PCoreQueueItemBase> {
    ...
    queue_key_map: DashMap<u32, QPStandardUniqueIdQueueKey<QUEUE_TOPIC_ID, QueueItem>>,
}
...
pub struct QueueGatherer<const QUEUE_TOPIC_ID: u32, QueueItem: PCoreQueueItemBase, Builder: QueueGathererItemBuilder + Send + Sync> {
    queue_key_helper: Arc<QueueKeyHelper<QUEUE_TOPIC_ID, QueueItem>>,
    _phantom: std::marker::PhantomData<Builder>,
}

impl<const QUEUE_TOPIC_ID: u32, QueueItem: PCoreQueueItemBase, Builder: QueueGathererItemBuilder + Send + Sync>
    QueueGatherer<QUEUE_TOPIC_ID, QueueItem, Builder>
{
    pub fn new(queue_key_helper: Arc<QueueKeyHelper<QUEUE_TOPIC_ID, QueueItem>>) -> Self {
        Self {
            queue_key_helper,
...
async fn gatherer<const QUEUE_TOPIC_ID: u32, QueueItem: PCoreQueueItemBase, Sub: QStandardEphemeralQueueSubscriber, Builder: QueueGathererItemBuilder + Send + Sync>(
    stream: Arc<Sub>,
    ...
    // The main cycle loop
    loop {
        let mut builder = Builder::create_new().await?;
        ...
        'gathering: loop {
            tokio::select! {
                ...
                maybe_msgs =     stream.dump_entire_ephemeral_queue_bytes(&queue_key, queue_key.realm_id, queue_key.realm_sub_id, queue_key.unique_id, queue_key.task_group as u32, 100_000) => {
                    ...
                    match maybe_msgs {
                        Ok(msgs) => {
                            if msgs.is_empty() {
                                ...
                                info!("GATHERER: Received {} messages from queue.", msgs.len());
                                for msg in msgs {
                                    builder.update_from_queue_item(msg).await?;
                                }
...
async fn processor<const QUEUE_TOPIC_ID: u32, QueueItem: PCoreQueueItemBase, Builder: QueueGathererItemBuilder + Send + Sync>(trigger_tx: mpsc::Sender<oneshot::Sender<Builder>>, queue_key_helper: Arc<QueueKeyHelper<QUEUE_TOPIC_ID, QueueItem>>) {
...
/// The Processor's main function.
async fn fake_api(client: Arc<NatsJetStreamClient>, queue_key_helper: Arc<QueueKeyHelper<USER_INFO_TEST_QUEUE_TOPIC_ID, QProvingJobDataID>>) -> anyhow::Result<()> {
    loop {
...
async fn main() -> anyhow::Result<()> {
    ...
    let gatherer_handle = tokio::spawn(gatherer::<USER_INFO_TEST_QUEUE_TOPIC_ID, QProvingJobDataID, _, SimpleDataBuilder>(stream.clone(), queue_key, queue_key_helper.clone(), trigger_rx));
    let processor_handle = tokio::spawn(processor::<USER_INFO_TEST_QUEUE_TOPIC_ID, QProvingJobDataID, SimpleDataBuilder>(trigger_tx, queue_key_helper.clone()));
