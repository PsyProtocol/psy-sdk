...
#[derive(Debug, Clone)]
pub struct PsyDashRecorderMemoryMerkleStore<Hasher, Hash: Copy + PartialEq + Default> {
    nodes: DashMap<SimpleMerkleNodeKey, Hash>,
    updated_nodes: DashMap<SimpleMerkleNodeKey, Hash>,
    height: u8,
...
impl<Hasher: MerkleZeroHasher<Hash>, Hash: Copy + PartialEq + Default + std::fmt::Debug>
    PsyDashRecorderMemoryMerkleStore<Hasher, Hash>
{
    ...
    pub fn get_changes(&self) -> &DashMap<SimpleMerkleNodeKey, Hash> {
        &self.updated_nodes
    }

    pub fn commit_changes(&self) {
        for entry in self.updated_nodes.iter() {
            self.nodes.insert(*entry.key(), *entry.value());
        }
    ...
    PsyDashRecorderMemoryMerkleStore<Hasher, Hash>
{
    ...
    fn get_node_value(&self, key: &SimpleMerkleNodeKey) -> Hash {
        match self.updated_nodes.get(key) {
            Some(v) => *v,
            None => match self.nodes.get(key) {
                Some(v) => *v,
...
pub fn get_merkle_proofs_for_compact_imm<Hasher: MerkleZeroHasher<Hash>, Hash: Copy + PartialEq + Default + std::fmt::Debug>(
    ...
    let tree = PsyDashRecorderMemoryMerkleStore::<Hasher, Hash>::new(height);
...
mod tests {
    ...
    type TestMerkleStore = PsyDashRecorderMemoryMerkleStore<TestHasher, TestHash>;
