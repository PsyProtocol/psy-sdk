...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::Worker { config, private_key, keystore_path, wallet_password, non_interactive } => {
            worker::run(config, private_key, keystore_path, wallet_password, non_interactive).await?;
        }
    };
    Ok::<_, anyhow::Error>(())
}
