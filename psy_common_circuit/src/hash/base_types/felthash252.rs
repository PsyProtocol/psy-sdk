...
pub trait CircuitBuilderFelt252Hash<F: RichField + Extendable<D>, const D: usize> {
    ...
    fn hash256_bytes_to_felt252_hashout_packed(&mut self, value: Hash256BytesTarget) -> HashOutTarget;
    ...
    fn connect_full_hashout_to_felt252_hashout(&mut self, standard_hashout: HashOutTarget, felt252_hashout: HashOutTarget);
    fn ensure_is_zero_or_top_bit(&mut self, value: Target);
}

impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilderFelt252Hash<F, D> for CircuitBuilder<F, D> {
    fn hash256_bytes_to_felt252_hashout(&mut self, value: Hash256BytesTarget) -> HashOutTarget {
        let results = value.iter().flat_map(|v| self.split_le(*v, 8)).collect::<Vec<_>>();
        let r2 = results.chunks_exact(64).map(|chunk| self.le_sum(chunk[0..63].iter())).collect::<Vec<_>>();
    ...
    fn hash256_bytes_to_felt252_hashout_packed(&mut self, value: Hash256BytesTarget) -> HashOutTarget {
        ...
        HashOutTarget { elements: [a, b, c, d] }
    }
