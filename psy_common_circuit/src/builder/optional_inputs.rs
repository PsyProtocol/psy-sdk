...
pub trait CircuitBuilderOptionalInputs<F: RichField + Extendable<D>, const D: usize> {
    ...
    fn add_virtual_hashes_if_none_op(
        &mut self,
        hash_targets: Option<Vec<HashOutTarget>>,
        length: usize,
        option_flags: &mut u64,
        flag: u64,
    ) -> Vec<HashOutTarget>;
}

impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilderOptionalInputs<F, D> for CircuitBuilder<F, D> {
    fn add_virtual_target_if_none(&mut self, target: Option<Target>) -> Target {
        if target.is_some() {
            target.unwrap()
        } else {
