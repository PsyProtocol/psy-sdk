...
impl SubTreeNodeStateTransitionGadget {
    ...
    pub fn set_witness<W: Witness<F>, F: RichField>(&self, witness: &mut W, transition: &SubTreeNodeStateTransition<F>) -> anyhow::Result<()> {
        witness.set_hash_target(self.old_node_value, transition.old_node_value.0)?;
        witness.set_hash_target(self.new_node_value, transition.new_node_value.0)?;
