...
use crate::nimpl::worker_queue_redis::redis_queue::{QueueNotification, RedisQueue};

use super::rabbit_stream_queue::{RabbitStreamQueue, Q_NOTIFICATIONS};



pub struct QEDRabbitStreamEventProcessor {
    pub job_queue: RabbitStreamQueue,
    pub redis_job_queue: RedisQueue,
    pub benckmarks_enabled: bool,
    pub benchmarks: Vec<QWorkerJobBenchmark>,
}
impl QEDRabbitStreamEventProcessor {
    pub fn new(dispatcher: RabbitStreamQueue, redis_job_queue: RedisQueue) -> Self {
        Self::new_with_config(dispatcher, redis_job_queue, false)
    }
    pub fn new_with_config(dispatcher: RabbitStreamQueue, redis_job_queue: RedisQueue, benckmarks_enabled: bool) -> Self {
        Self {
...
#[async_trait]
impl WorkerEventReceiverAsync for QEDRabbitStreamEventProcessor {
...
#[async_trait]
impl WorkerEventTransmitterAsync for QEDRabbitStreamEventProcessor {
    async fn enqueue_jobs_mut(&mut self, jobs: &[QProvingJobDataID]) -> anyhow::Result<()> {
        self.job_queue.push_tasks(jobs).await?;
        Ok(())
    }
