...
impl BlockSyncService {
    ...
    async fn sync_once(&self) -> Result<u64> {
        timeout(
            Duration::from_secs(BLOCK_SYNC_TIMEOUT_SECS),
    ...
    async fn fetch_block_height(&self) -> Result<u64> {
        let fetch_fn = match self.node_type {
            WatcherSourceNodeType::Coordinator => QEDCoordinatorStoreReaderAsync::get_latest_l2_block_state,
            WatcherSourceNodeType::Realm => QEDRealmStoreReaderAsync::get_latest_l2_block_state,
        };

        let block_state = fetch_fn(&self.qed_store)
            .await
