...
use qed_data::{
    ...
    guta::{
        ...
        proof_input::{GUTANoChangeFullInput, VerifyGUTAToCapCircuitInputSimple, VerifyGUTAToCapUpgradeCheckpointCircuitInputSimple, VerifyTwoGUTAProofGadgetStandardInputSimple, VerifyTwoGUTAProofUpgradeCheckpointStandardInputSimple},
        stats::GUTAStats,
    },
    ...
    > CoordinatorProcessorContext<SR, DQ, HQ, WQ, PS, TS>
{
    ...
    pub async fn handle_deploy_contracts(&self, checkpoint_id: u64) -> anyhow::Result<(Vec<Vec<QProvingJobDataID>>, AggStateTransition<F>, u32, BidirectionalGraph<QProvingJobDataID>)> {
        ...
        let wits = spiderman_append_proofs
            ...
            .map(|(i, (spiderman_proof, start_idx))| {
                ...
                self.push_deploy_contracts_request(checkpoint_id, self.coordinator_config.coordinator_id, i as u32, &mut psb, spiderman_proof, contract_leaves)
            })
    ...
    pub async fn handle_guta_from_realms(
        ...
        let mut guta_queue_items = guta_queue_items.into_iter().filter(|x| {
            warn!("Filtering GUTA queue items for checkpoint_id: {}, current item checkpoint_id: {}", checkpoint_id, x.checkpoint_id);
            x.checkpoint_id < checkpoint_id
        }).collect::<Vec<_>>();
