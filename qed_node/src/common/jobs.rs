...
impl JobReceiver for JobClient {
    async fn get_next_job(&self, wallet: Arc<Wallet>) -> anyhow::Result<QJob> {
        loop {
            let signed_request = wallet.sign_claim_job()?;
    ...
    async fn submit_job_proof(
        ...
        let signed = wallet.sign_proof_submission(job, proof)?;
        JobSchedulerRpcClient::set_proof_by_id(&self.rpc_client, signed).await?;
        Ok(())
    }
}
