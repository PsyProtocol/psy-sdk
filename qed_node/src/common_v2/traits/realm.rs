...
use psy_core::{config::network_constants::{GLOBAL_USER_TREE_HEIGHT, COORDINATOR_USER_TREE_HEIGHT}, data::qhashout::QHashOut, job::id::{ProvingJobCircuitType, ProvingJobDataType, QJobTopic, QProvingJobDataID, QProvingTask, QProvingJobGraph}, utils::graph::BidirectionalGraph};
...
pub struct RealmDataForCoordinatorHeader<F: RichField> {
    // todo fill in data here to send to coordindator, like the proof that proves the realm root updates from its old value to a new value and witness info
    pub realm_id: u64,
...
pub struct RealmDataForCoordinator<F: RichField> {
    // todo fill in data here to send to coordindator, like the proof that proofs the realm root updates from its old value to a new value and witness info
...
impl<F: RichField> SimpleTreeUpdateBuilder<F> {
    ...
    pub fn add_update(&mut self, level: u8, index: u64, new_value: QHashOut<F>) {
        self.updates.push(GenericTreeNodeUpdate {
            level,
            index,
            new_value,
        });
...
fn build_delta_merkle_proof_and_record<F: RichField, H: MerkleHasher<QHashOut<F>>>(
    ...
    let (_, new_root) = compute_root_delta_merkle_proof_generic_record::<F,H>(tree_update_builder, node.new_value, node.index, &merkle_proof.siblings);
...
pub trait GlobalUserTreeMerkleReader<F: RichField> {
    ...
    async fn resolve_delta_merkle_proofs_for_nca<H: MerkleHasher<QHashOut<F>>>(
        ...
        let (nca_index, left_merkle_proof) = match self.get_sub_tree_merkle_proof::<H>(checkpoint_id, left_node.level, left_node.index, nca_computed_key.level).await {
            Ok((nca_index, left_merkle_proof)) => (nca_index, left_merkle_proof),
            Err(e) => return Err(e),
        };
        let (nca_index_right, mut right_merkle_proof) = match self.get_sub_tree_merkle_proof::<H>(checkpoint_id, right_node.level, right_node.index, nca_computed_key.level).await {
        ...
        if nca_index != nca_index_right || nca_computed_key.index != nca_index {
            anyhow::bail!("NCA index mismatch between left and right proofs: left {}, right {}, computed: {}, this should never happen", nca_index, nca_index_right, nca_computed_key.index);
        }

        let (left_second_to_last, left_root) = compute_root_delta_merkle_proof_generic_record::<F, H>(tree_update_builder, left_node.new_value, left_node.index, &left_merkle_proof.siblings);

        if to_level == nca_computed_key.level {
            // Handle edge case: if right node is at NCA level, OR if left node is at NCA level
            let (right_siblings_for_computation, right_root) = if right_node.level == nca_computed_key.level {
                ...
                let (_, right_root) = compute_root_delta_merkle_proof_generic_record::<F, H>(tree_update_builder, right_node.new_value, right_node.index, &right_merkle_proof.siblings);
                ...
                // Normal case: both nodes need path to NCA, modify siblings for sequential update
                ...
                let (_, right_root) = compute_root_delta_merkle_proof_generic_record::<F, H>(tree_update_builder, right_node.new_value, right_node.index, &right_siblings);
            ...
            let right_delta_merkle_proof = DeltaMerkleProofCore {
                ...
                old_root: if right_node.level == nca_computed_key.level {
                    right_merkle_proof.root  // Right node is NCA, use its original root
                } else if left_node.level == nca_computed_key.level {
                    right_merkle_proof.root  // Left node is at NCA, right uses its original root
                } else {
                    left_root  // Normal case: should be based on left's updated state
                },
                index: right_node.index,
            };
            ...
            let (real_index, top_proof) = match self.get_sub_tree_merkle_proof::<H>(checkpoint_id, nca_computed_key.level, nca_computed_key.index, to_level).await {
            ...
            // Only apply if both left and right nodes need paths to NCA (neither is at NCA level)
            if right_merkle_proof.siblings.len() > 0 &&
               left_node.level != nca_computed_key.level &&
               right_node.level != nca_computed_key.level {
                // The NCA level sibling is still at the same relative position in the extended path
                // It's at index (right_merkle_proof.siblings.len() - 1) in the extended right_siblings
            ...
            let (_, new_right_root) = compute_root_delta_merkle_proof_generic_record::<F, H>(tree_update_builder, right_node.new_value, right_node.index, &right_siblings);
            let right_delta_proof = DeltaMerkleProofCore {
                ...
                old_root: if right_node.level == nca_computed_key.level {
                    top_proof.root  // Right node is original NCA, use original target root
                } else if left_node.level == nca_computed_key.level {
                    top_proof.root  // Left node is at NCA level, no sequential update applied
                } else {
    ...
    async fn get_nca_delta_merkle_proof<H: MerkleHasher<QHashOut<F>>>(
        ...
        let left_node_key = SimpleMerkleNodeKey { level: left_node.level, index: left_node.index };
        let right_node_key = SimpleMerkleNodeKey { level: right_node.level, index: right_node.index };
        let nca_computed_key = left_node_key.find_nearest_common_ancestor(&right_node_key);

        let to_level = to_level.unwrap_or(nca_computed_key.level);

        tracing::debug!("🔍 left_node: {:?}, right_node: {:?}, nca_computed_key: {:?}, to_level: {:?}",
            left_node, right_node, nca_computed_key, to_level);

        self.resolve_delta_merkle_proofs_for_nca::<H>(tree_update_builder, checkpoint_id, left_node, right_node, to_level).await
    }
    ...
    async fn get_single_node_delta_merkle_proof<H: MerkleHasher<QHashOut<F>>>(
        ...
        if nca_index != expected_nca_index {
            anyhow::bail!("NCA index for single node proof should be {} (calculated from index {} level {} to level {}), got {}",
                         expected_nca_index, node.index, node.level, to_level, nca_index);
        }
...
pub trait CoordinatorClient<F: RichField> {
    ...
    // [from, to] inclusive, ie. from and to can be the same value and it returns a record, calling with get_latest_block_updates_from_coordinator(40,41) returns 40 AND 41 if they exist
    async fn get_latest_block_updates_from_coordinator(&self, realm_id: u64, from_checkpoint: u64, to_checkpoint: u64) -> anyhow::Result<Vec<GlobalBlockUpdateFromCoordinator<F>>>;
...
pub trait RealmProcessorStateClient<F: RichField>: GlobalUserTreeMerkleReader<F> {
    // a unique checkpoint id shared by realm processors and all edges, processor can write, but edges can only read
    ...
    async fn load_update_delta_records(&self, realm_id: u64, target_realm_root: QHashOut<F>) -> anyhow::Result<Option<RealmProcessorCombinedUpdate<F>>>;
    ...
    // gets all the updates to apply from peers to sync the node to latest, should first try to use load_update_delta_records incase the node crashed during writing the state
    async fn sync_latest_realm_deltas_from_peers(&self, realm_id: u64, from_realm_root: QHashOut<F>, to_realm_root: QHashOut<F>) -> anyhow::Result<Vec<RealmProcessorCombinedUpdate<F>>>;
...
impl<F: RichField, R: QEDRealmStoreReaderAsync<F> + Sync> GlobalUserTreeMerkleReader<F> for R {
    async fn get_sub_tree_merkle_proof<H: MerkleHasher<QHashOut<F>>>(
            ...
            let merkle_proof = self.get_user_sub_tree_merkle_proof(
                checkpoint_id,
                to_level,
                from_level,
                from_index
...
impl GraphDependencyBuilder for QProvingTaskStoreImpl {
    ...
    async fn finish(&self, checkpoint_id: u64, realm_id: u32) -> anyhow::Result<()> {
        ...
        let tasks: Vec<QProvingTask> = levels.iter()
...
mod tests {
    ...
    impl GlobalUserTreeMerkleReader<GoldilocksField> for InMemoryGlobalUserTreeMerkleReader {
        ...
        async fn get_multiple_delta_merkle_proofs<H: MerkleHasher<QHashOut<GoldilocksField>>>(
            ...
            to_level: Option<u8>,
        ) -> anyhow::Result<(GenericTreeNodeUpdate<GoldilocksField>, Vec<DeltaMerkleProofCore<QHashOut<GoldilocksField>>>)> {
            unimplemented!()
        }
    }
    ...
    async fn test_get_single_node_delta_merkle_proof() -> anyhow::Result<()> {
        ...
        let result = reader.get_single_node_delta_merkle_proof::<PoseidonHasher>(&mut builder, 0, test_node, 0).await?;
    ...
    async fn test_get_single_node_delta_merkle_proof_to_level() -> anyhow::Result<()> {
        ...
        let result = reader.get_single_node_delta_merkle_proof::<PoseidonHasher>(&mut builder, 0, test_node, COORDINATOR_USER_TREE_HEIGHT).await?;
        println!("result: {}", serde_json::to_string_pretty(&result).unwrap());
        ...
        assert_eq!(delta_proof.index, test_node.index);
        assert_eq!(delta_proof.new_value, test_node.new_value);
        assert_eq!(delta_proof.siblings.len(), (GLOBAL_USER_TREE_HEIGHT - COORDINATOR_USER_TREE_HEIGHT) as usize);
    ...
    async fn test_get_nca_delta_merkle_proof_force_to_root_false() -> anyhow::Result<()> {
        ...
        let left_key = SimpleMerkleNodeKey { level: left_node.level, index: left_node.index };
        let right_key = SimpleMerkleNodeKey { level: right_node.level, index: right_node.index };
        ...
        let result = reader.get_nca_delta_merkle_proof::<PoseidonHasher>(
            &mut builder, 0, left_node, right_node, None
        ).await?;
        println!("result: {}", serde_json::to_string_pretty(&result).unwrap());
        ...
        // For sequential updates, the NCA new_value should be the right proof's new_root
        // since right proof is computed based on left's updated state
        assert_eq!(nca_update.new_value, right_proof.new_root, "NCA new_value should be right_proof.new_root for sequential updates");
    ...
    async fn test_get_nca_delta_merkle_proof_force_to_root_true() -> anyhow::Result<()> {
        ...
        let result = reader.get_nca_delta_merkle_proof::<PoseidonHasher>(
            &mut builder, 0, left_node, right_node, Some(0)
        ).await?;
        ...
        assert_eq!(left_proof.siblings.len(), GLOBAL_USER_TREE_HEIGHT as usize, "Left proof should go to root");
        assert_eq!(right_proof.siblings.len(), GLOBAL_USER_TREE_HEIGHT as usize, "Right proof should go to root");
    ...
    // REMOVED: test_nca_index_consistency - Successfully proved the NCA index mismatch bug
    // This test confirmed that using each other's level as to_level causes "NCA index mismatch"
    // The correct approach is to use nca_computed_key.level as to_level for both calls
    ...
    async fn test_nca_index_consistency_disabled() -> anyhow::Result<()> {
        ...
        for (left_level, left_idx, right_level, right_idx, description) in test_cases {
            ...
            let left_key = SimpleMerkleNodeKey { level: left_level, index: left_idx };
            let right_key = SimpleMerkleNodeKey { level: right_level, index: right_idx };
            ...
            if left_level == GLOBAL_USER_TREE_HEIGHT {
                ...
                tree.set_node_value(SimpleMerkleNodeKey { level: left_level, index: left_idx }, left_value);
            }
            ...
            if right_level == GLOBAL_USER_TREE_HEIGHT {
                ...
                tree.set_node_value(SimpleMerkleNodeKey { level: right_level, index: right_idx }, right_value);
            }
            ...
            // get_sub_tree_merkle_proof(right_node.level, right_node.index, left_node.level)
            ...
            let (nca_index_left, _) = reader.get_sub_tree_merkle_proof::<PoseidonHasher>(
                0,
                left_level,
                left_idx,
                safe_to_level
            ...
            let (nca_index_right, _) = reader.get_sub_tree_merkle_proof::<PoseidonHasher>(
                0,
                right_level,
                right_idx,
                safe_to_level
            ...
            if nca_index_left != nca_index_right || nca_computed_key.index != nca_index_left {
                ...
                println!("  nca_index_left={}, nca_index_right={}, nca_computed_key.index={}",
            ...
            let (nca_index_left_correct, _) = reader.get_sub_tree_merkle_proof::<PoseidonHasher>(
                0,
                left_level,
                left_idx,
                nca_computed_key.level
            ).await?;

            let (nca_index_right_correct, _) = reader.get_sub_tree_merkle_proof::<PoseidonHasher>(
                0,
                right_level,
                right_idx,
                nca_computed_key.level
    ...
    async fn test_nca_index_mismatch_fix() -> anyhow::Result<()> {
        ...
        for (left_idx, right_idx) in test_cases {
            // Calculate expected NCA
            let left_key = SimpleMerkleNodeKey { level: GLOBAL_USER_TREE_HEIGHT, index: left_idx };
            let right_key = SimpleMerkleNodeKey { level: GLOBAL_USER_TREE_HEIGHT, index: right_idx };
            let nca_computed_key = left_key.find_nearest_common_ancestor(&right_key);
            ...
            println!("  Correct approach (to_level={}): left_return={}, right_return={}",
                     correct_to_level, left_return_correct, right_return_correct);
            println!("  Wrong approach (to_level={}): left_return={}, right_return={}",
                     wrong_to_level, left_return_wrong, right_return_wrong);

            // Verify fix works
            assert_eq!(left_return_correct, nca_computed_key.index,
                      "Left return should match NCA index with correct to_level");
            assert_eq!(right_return_correct, nca_computed_key.index,
                      "Right return should match NCA index with correct to_level");

            // Verify old approach was wrong (unless special case where nca is at leaf level)
            if correct_to_level != wrong_to_level {
                assert!(left_return_wrong != nca_computed_key.index ||
                       right_return_wrong != nca_computed_key.index,
    ...
    async fn test_get_sub_tree_merkle_proof_different_levels_and_indices() -> anyhow::Result<()> {
        ...
        for (from_level, from_index, to_level, expected_to_index) in test_cases {
            ...
            let result = reader.get_sub_tree_merkle_proof::<PoseidonHasher>(0, from_level, from_index, to_level).await?;
            println!("result: {}", serde_json::to_string_pretty(&result).unwrap());
            ...
            let (returned_index, proof) = result;
                    ...
                    assert_eq!(proof.siblings.len(), expected_siblings_count,
                              "Proof should have {} siblings for levels {} to {}",
                              expected_siblings_count, from_level, to_level);
    ...
    async fn test_comprehensive_nca_scenarios() -> anyhow::Result<()> {
        ...
        for (left_index, right_index, force_to_root, description) in scenarios {
            ...
            let result = reader.get_nca_delta_merkle_proof::<PoseidonHasher>(
                &mut builder, 0, left_node, right_node, if force_to_root { Some(0) } else { None }
            ).await;
    ...
    async fn test_diverse_level_index_combinations() -> anyhow::Result<()> {
        ...
        // Test cases: (left_level, left_index, right_level, right_index, force_to_root, description)
        let test_cases = vec![
            // Adjacent leaves - realistic case
            (GLOBAL_USER_TREE_HEIGHT, 0, GLOBAL_USER_TREE_HEIGHT, 1, false, "Adjacent leaves, NCA level"),
            (GLOBAL_USER_TREE_HEIGHT, 0, GLOBAL_USER_TREE_HEIGHT, 1, true, "Adjacent leaves, force to root"),

            // Distant leaves - test NCA computation
            (GLOBAL_USER_TREE_HEIGHT, 16, GLOBAL_USER_TREE_HEIGHT, 24, false, "Distant leaves, NCA level"),
            (GLOBAL_USER_TREE_HEIGHT, 16, GLOBAL_USER_TREE_HEIGHT, 24, true, "Distant leaves, force to root"),

            // Same level, wide separation
            (GLOBAL_USER_TREE_HEIGHT, 0, GLOBAL_USER_TREE_HEIGHT, 1023, false, "Far apart leaves, NCA level"),
            (GLOBAL_USER_TREE_HEIGHT, 0, GLOBAL_USER_TREE_HEIGHT, 1023, true, "Far apart leaves, force to root"),
            ...
            // Close indices
            (GLOBAL_USER_TREE_HEIGHT, 64, GLOBAL_USER_TREE_HEIGHT, 65, false, "Adjacent leaves, NCA level"),
            (GLOBAL_USER_TREE_HEIGHT, 64, GLOBAL_USER_TREE_HEIGHT, 65, true, "Adjacent leaves, force to root"),

            // Power of 2 boundaries
            (GLOBAL_USER_TREE_HEIGHT, 256, GLOBAL_USER_TREE_HEIGHT, 512, false, "Power of 2 boundaries, NCA level"),
            (GLOBAL_USER_TREE_HEIGHT, 256, GLOBAL_USER_TREE_HEIGHT, 512, true, "Power of 2 boundaries, force to root"),
        ];
        ...
        for (left_level, left_index, right_level, right_index, force_to_root, description) in test_cases {
            ...
            // Calculate expected NCA
            let left_key = SimpleMerkleNodeKey { level: left_level, index: left_index };
            let right_key = SimpleMerkleNodeKey { level: right_level, index: right_index };
            let expected_nca = left_key.find_nearest_common_ancestor(&right_key);
            ...
            match result {
                Ok((nca_update, left_proof, right_proof)) => {
                    ...
                    // For sequential updates, the NCA new_value should be the right proof's new_root
                    // since right proof is computed based on left's updated state
                    assert_eq!(nca_update.new_value, right_proof.new_root,
                              "NCA new_value should be right_proof.new_root for sequential updates in case: {}", description);

                    // Verify proof correctness - especially important for to_level != nca_level cases
                    if !left_proof.verify::<PoseidonHasher>() {
                        println!("❌ Left proof verification failed for case: {}", description);
                        println!("Left proof: old_root={:?}, new_root={:?}, index={}, siblings={}",
                                left_proof.old_root, left_proof.new_root, left_proof.index, left_proof.siblings.len());
                    }
                    if !right_proof.verify::<PoseidonHasher>() {
                        println!("❌ Right proof verification failed for case: {}", description);
                        println!("Right proof: old_root={:?}, new_root={:?}, index={}, siblings={}",
                                right_proof.old_root, right_proof.new_root, right_proof.index, right_proof.siblings.len());
                        println!("Right proof details: old_value={:?}, new_value={:?}",
                                right_proof.old_value, right_proof.new_value);
                        println!("Case details: left=({},{}), right=({},{}), NCA=({},{}), force_to_root={}",
                                left_level, left_index, right_level, right_index, nca_update.level, nca_update.index, force_to_root);
                    }
                    assert!(left_proof.verify::<PoseidonHasher>(), "left_proof should be valid for case: {}", description);
                    assert!(right_proof.verify::<PoseidonHasher>(), "right_proof should be valid for case: {}", description);

                    println!("  builder.updates: {}", serde_json::to_string_pretty(&builder.updates).unwrap());
                    println!("  ✅ Success: nca_level={}, nca_index={}, expected_nca_level={}",
    ...
    async fn test_extreme_cases() -> anyhow::Result<()> {
        ...
        let extreme_cases = vec![
            ...
            (GLOBAL_USER_TREE_HEIGHT, (1u64 << GLOBAL_USER_TREE_HEIGHT) - 1, GLOBAL_USER_TREE_HEIGHT, 0, false, "Max index vs 0, NCA level"),
            (GLOBAL_USER_TREE_HEIGHT, (1u64 << GLOBAL_USER_TREE_HEIGHT) - 1, GLOBAL_USER_TREE_HEIGHT, 0, true, "Max index vs 0, force to root"),
        ];
        ...
        for (left_level, left_index, right_level, right_index, force_to_root, description) in extreme_cases {
            ...
            match result {
                Ok((nca_update, left_proof, right_proof)) => {
                    ...
                    assert!(left_proof.verify::<PoseidonHasher>(), "left_proof should be valid for extreme case: {}", description);
                    assert!(right_proof.verify::<PoseidonHasher>(), "right_proof should be valid for extreme case: {}", description);
    ...
    async fn test_siblings_ordering_verification() -> anyhow::Result<()> {
        ...
        let (nca_update, left_proof, right_proof) = result;
                ...
                // Verify proof structure
                assert_eq!(left_proof.siblings.len(), GLOBAL_USER_TREE_HEIGHT as usize,
                          "Left proof should have {} siblings to reach root", GLOBAL_USER_TREE_HEIGHT);
                assert_eq!(right_proof.siblings.len(), GLOBAL_USER_TREE_HEIGHT as usize,
                          "Right proof should have {} siblings to reach root", GLOBAL_USER_TREE_HEIGHT);
                ...
                let computed_root_left = compute_root_merkle_proof_generic::<QHashOut<GoldilocksField>, PoseidonHasher>(
                    left_proof.new_value, left_proof.index, &left_proof.siblings
                );
                let computed_root_right = compute_root_merkle_proof_generic::<QHashOut<GoldilocksField>, PoseidonHasher>(
                    right_proof.new_value, right_proof.index, &right_proof.siblings
                );
    ...
    async fn test_tree_update_consistency() -> anyhow::Result<()> {
        // This test verifies that applying accumulated updates produces the same result as direct set operations
        println!("Testing tree update consistency...");

        let test_cases = vec![
            // (left_level, left_index, right_level, right_index, force_to_root, description)
            (GLOBAL_USER_TREE_HEIGHT, 16, GLOBAL_USER_TREE_HEIGHT, 24, false, "Adjacent leaves, NCA level"),
            (GLOBAL_USER_TREE_HEIGHT, 16, GLOBAL_USER_TREE_HEIGHT, 24, true, "Adjacent leaves, force to root"),
            (GLOBAL_USER_TREE_HEIGHT, 100, GLOBAL_USER_TREE_HEIGHT, 200, false, "Distant leaves, NCA level"),
            (GLOBAL_USER_TREE_HEIGHT, 100, GLOBAL_USER_TREE_HEIGHT, 200, true, "Distant leaves, force to root"),
        ];
        ...
        for (left_level, left_index, right_level, right_index, force_to_root, description) in test_cases {
            ...
            for update in &builder.updates {
                tree_via_updates.set_node_value(
                    SimpleMerkleNodeKey { level: update.level, index: update.index },
                    update.new_value
                );
            }
            ...
            tree_via_updates.set_node_value(
                SimpleMerkleNodeKey { level: nca_update.level, index: nca_update.index },
            ...
            if left_level == GLOBAL_USER_TREE_HEIGHT {
                ...
                tree_via_direct_set.set_node_value(
                    SimpleMerkleNodeKey { level: left_level, index: left_index },
                    left_node.new_value
                );
            }
            ...
            if right_level == GLOBAL_USER_TREE_HEIGHT {
                ...
                tree_via_direct_set.set_node_value(
                    SimpleMerkleNodeKey { level: right_level, index: right_index },
            ...
            assert_eq!(root_via_updates, root_via_direct_set,
            ...
            let target_key = SimpleMerkleNodeKey { level: nca_update.level, index: nca_update.index };
            ...
            assert_eq!(value_via_updates, value_via_direct_set,
                      "Target node value mismatch for case '{}' at level={}, index={}",
                      description, nca_update.level, nca_update.index);

            println!("  ✅ Consistency verified: {} accumulated updates produced same result as direct set", builder.updates.len());
        }
    ...
    async fn test_nca_delta_merkle_proof_fuzzy() -> anyhow::Result<()> {
        ...
        for i in 0..num_tests {
            ...
            let right_index = if max_right_index == 0 { 0 } else { next_random() % (max_right_index + 1) };
            ...
            match reader.get_nca_delta_merkle_proof::<PoseidonHasher>(
                &mut builder, 0, left_node, right_node, if force_to_root { Some(0) } else { None }
            ).await {
                Ok((nca_update, left_proof, right_proof)) => {
                    // Verify proof correctness
                    ...
                    if left_valid && right_valid {
                        ...
                        if i < 10 { // Print first few successful cases for debug
                            println!("✅ Test {}: left=({},{}), right=({},{}), force_to_root={}, NCA=({},{})",
                                    i, left_level, left_index, right_level, right_index, force_to_root,
                                    nca_update.level, nca_update.index);
                        }
                    } else {
                        failed += 1;
                        println!("❌ Test {}: Proof verification failed - left=({},{}), right=({},{}), force_to_root={}",
                                i, left_level, left_index, right_level, right_index, force_to_root);
                        ...
                        if failed <= 5 {
                            if !left_valid {
                                println!("   Left proof: siblings={}, old_root={:?}, new_root={:?}",
                                        left_proof.siblings.len(), left_proof.old_root, left_proof.new_root);
                            }
                            if !right_valid {
                                println!("   Right proof: siblings={}, old_root={:?}, new_root={:?}",
                                        right_proof.siblings.len(), right_proof.old_root, right_proof.new_root);
                            }
                        }
                    }
                }
                Err(e) => {
                    failed += 1;
                    println!("❌ Test {}: get_nca_delta_merkle_proof failed - left=({},{}), right=({},{}), force_to_root={}: {}",
        ...
        assert!(success_rate >= 0.8, "Success rate too low: {:.1}% (expected >= 80%)", success_rate * 100.0);
    ...
    async fn test_debug_failing_cases() -> anyhow::Result<()> {
        ...
        for (i, (left_level, left_index, right_level, right_index, force_to_root, description)) in failing_cases.iter().enumerate() {
            ...
            println!("left=({},{}), right=({},{}), force_to_root={}",
            ...
            match reader.get_nca_delta_merkle_proof::<PoseidonHasher>(
                ...
                Ok((nca_update, left_proof, right_proof)) => {
                    ...
                    if !left_valid {
                        println!("Left proof details: old_value={:?}, new_value={:?}, old_root={:?}, new_root={:?}",
                                left_proof.old_value, left_proof.new_value, left_proof.old_root, left_proof.new_root);
                    }

                    if !right_valid {
                        println!("Right proof details: old_value={:?}, new_value={:?}, old_root={:?}, new_root={:?}",
    ...
    async fn test_sequential_update_consistency() -> anyhow::Result<()> {
        ...
        for (left_index, right_index, force_to_root, description) in test_cases {
            ...
            assert_eq!(initial_root, intermediate_root,
                      "Initial roots should match for case '{}'", description);

            // Verify final states match
            assert_eq!(final_root_via_updates, final_root_via_direct_sets,
            ...
            assert_eq!(nca_update.new_value, expected_nca_value,
                      "NCA update value should match expected for case '{}'", description);
