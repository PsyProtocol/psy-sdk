    ...
    AlgebraicHasher<C::F> + MerkleZeroHasher<HashOut<C::F>> + MerkleZeroHasher<QHashOut<C::F>>,
{
    pub fn submit_proof(&mut self, proof_input: SubmitUserEndCapProofAPIInput<F, C, D>) -> anyhow::Result<()>{
        ...
        for u in proof_input.input.contract_state_updates.iter() {
            ...
            for dmp in u.contract_state_tree_updates.iter() {
                self.state_store.set_user_state_tree_leaf_hash(new_checkpoint_id, user_id, contract_id, height, dmp.index, dmp.new_value)?;
            }
        }
    ...
    pub async fn get_start_witnesses(&mut self) -> anyhow::Result<(Vec<VerifyTwoEndCapCircuitWithIdsInput<F>>,Option<SubmitUserEndCapProofIDAPIInput<F>>)>  {
        ...
        results.sort_by(|a,b| a.input.core.state_transition.user_id.to_canonical_u64().cmp(&b.input.core.state_transition.user_id.to_canonical_u64()));
        ...
        for i in 0..good_pairs {
            ...
            let input = VerifyTwoEndCapCircuitWithIdsInput {
                ...
                proof_a_id: QProvingJobDataID::end_cap_proof(self.latest_l2_block_state.checkpoint_id, 0, 0, results[i*2].input.core.state_transition.user_id.to_canonical_u64() as u32),
                proof_b_id: QProvingJobDataID::end_cap_proof(self.latest_l2_block_state.checkpoint_id, 0, 0, results[i*2+1].input.core.state_transition.user_id.to_canonical_u64() as u32),
            };
            verify_two_end_cap_inputs.push(input)
        }
        self.next_block_mempool_updates.clear();

        Ok((verify_two_end_cap_inputs, if results.len() == good_pairs*2 {
            None
