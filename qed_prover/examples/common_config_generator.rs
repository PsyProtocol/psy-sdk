...
fn run_gen_config() -> anyhow::Result<()> {
    ...
    let library_path = my_home()?.unwrap().join("Projects/qedlang-rust/qed_crypto/src/common/cached_circuit_library.rs");
...
"##, library_data)?;
    ...
    let common_path = my_home()?.unwrap().join("Projects/qedlang-rust/qed_node_common/src/verifier/cached_common_data.rs");
