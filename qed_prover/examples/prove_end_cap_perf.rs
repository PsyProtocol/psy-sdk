    ...
    C::Hasher: AlgebraicHasher<C::F>,
{
    pub fn into_ser(self) -> DebugEndCapPerfInputSer<C, D> {
        DebugEndCapPerfInputSer {
            core: self.core,
            agg_root_verifier_data: AltVerifierOnlyCircuitData::new_from_verifier_data(
                &self.agg_root_verifier_data,
            ),
        }
    }
}
...
impl<C: GenericConfig<D>, const D: usize> From<DebugEndCapPerfInputSer<C, D>>
    for DebugEndCapPerfInputReady<C, D>
where
    C::Hasher: AlgebraicHasher<C::F>,
{
...
impl<C: GenericConfig<D>, const D: usize> From<DebugEndCapPerfInputReady<C, D>>
    for DebugEndCapPerfInputSer<C, D>
where
    C::Hasher: AlgebraicHasher<C::F>,
{
        ...
        AlgebraicHasher<C::F> + MerkleZeroHasher<HashOut<C::F>> + MerkleZeroHasher<QHashOut<C::F>>,
{
    ...
    pub fn run_bench_end_cap(
        &mut self,
        input_data: &DebugEndCapPerfInputReady<C, D>,
    ) -> anyhow::Result<()> {
...
fn main1() -> anyhow::Result<()> {
    ...
    let end_caps: Vec<DebugEndCapPerfInputSer<C,D>> = vec![];//serde_json::from_str::<Vec<DebugEndCapPerfInputSer<C, D>>>(include_str!("./debug_data/end_cap_dbg_2.json"))?;
