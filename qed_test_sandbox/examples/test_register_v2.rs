...
async fn run_fred_test3() -> anyhow::Result<()> {
    ...
    let gj = SimpleAsyncRealmWorker::run_worker_until_done::<
        _,
        _,
    ...
    let r: GUTARealmCheckpointResult<QEDFelt>  = bincode::deserialize(&realm_qps.get_bytes_by_id(gj).await?).map_err(|e| anyhow::anyhow!("{:?}",e))?;
    let realm_proof = realm_qps.get_proof_by_id(r.proof_id).await?;

    coordinator_edge_node.ctx.handle_recv_guta_from_realm(SubmitGUTARealmResultAPINoProofInput{
        realm_id: 0,
        checkpoint_id: r.checkpoint_id,
        guta_stats: r.guta_stats,
        top_line_proof: r.top_line_proof,
        checkpoint_tree_root: r.checkpoint_tree_root,
        circuit_type:r.proof_id.circuit_type,
    }, &realm_proof).await?;
