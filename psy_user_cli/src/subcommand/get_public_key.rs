...
use psy_crypto::{
    ...
    signature::{
        ...
        zk::wallet::{SimpleL2PrivateKey, SimpleQEDPrivateKey},
    },
};
use psy_data::config::store_config::QEDHasher;
use psy_prover::{
    local::args::SignType,
    ups::circuit_manager::core::{QCircuitManager, QEDUPSStepCircuitManager},
    wallet::{memory_wallet::QEDMemoryWallet, secp_wallet::Wallet},
};
...
pub async fn run(args: GetPublicKeyArgs) -> anyhow::Result<()> {
    match args.sign_type {
        ...
        SignType::SECP256K1Sign => {
            ...
            let main_circuits = QCircuitManager::Local(QEDUPSStepCircuitManager::<C, D>::new_with_config(
