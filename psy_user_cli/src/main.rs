...
use psy_prover::local::native::prove_proxy;
...
use crate::subcommand::claim_amount;
use crate::subcommand::check_tx;
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::GetUserContractStateTreeRoot(args) => {
            ...
            let root = provider.get_user_contract_state_tree_root(args.checkpoint_id, args.user_id, args.contract_id).await?;
        ...
        Commands::GetUserContractStateTreeLeafHash(args) => {
            ...
            use psy_data::traits::qdatastore::qtreedata::QTreeDataStoreReaderSync;
            let provider = RpcProvider::new_with_config_path(&args.rpc_config)?;
            let hash = provider.get_user_contract_state_tree_leaf_hash(args.checkpoint_id, args.user_id, args.contract_id, args.height, args.leaf_id).await?;
        ...
        Commands::GetUserContractStateTreeMerkleProof(args) => {
            ...
            use psy_data::traits::qdatastore::qtreedata::QTreeDataStoreReaderSync;
            let provider = RpcProvider::new_with_config_path(&args.rpc_config)?;
            let proof = provider.get_user_contract_state_tree_merkle_proof(args.checkpoint_id, args.user_id, args.contract_id, args.height, args.leaf_id).await?;
        ...
        Commands::GetUserContractTreeLeafHash(args) => {
            ...
            use psy_data::traits::qdatastore::qtreedata::QTreeDataStoreReaderSync;
            let provider = RpcProvider::new_with_config_path(&args.rpc_config)?;
            let hash = provider.get_user_contract_tree_leaf_hash(args.checkpoint_id, args.user_id, args.contract_id).await?;
            println!("{}", serde_json::to_string_pretty(&hash)?);
        }
        Commands::GetUserContractTreeMerkleProof(args) => {
            ...
            use psy_data::traits::qdatastore::qtreedata::QTreeDataStoreReaderSync;
            let provider = RpcProvider::new_with_config_path(&args.rpc_config)?;
            let proof = provider.get_user_contract_tree_merkle_proof(args.checkpoint_id, args.user_id, args.contract_id).await?;
            println!("{}", serde_json::to_string_pretty(&proof)?);
        }
        ...
        Commands::GetUserRegistrationTreeMerkleProof(args) => {
            ...
            use psy_data::traits::qdatastore::qtreedata::QTreeDataStoreReaderSync;
            let provider = RpcProvider::new_with_config_path(&args.rpc_config)?;
            let proof = provider.get_user_registration_tree_merkle_proof(args.checkpoint_id, args.leaf_index).await?;
            println!("{}", serde_json::to_string_pretty(&proof)?);
        }
        ...
        Commands::GetUserSubTreeMerkleProof(args) => {
            ...
            use psy_data::traits::qdatastore::qtreedata::QTreeDataStoreReaderSync;
            let provider = RpcProvider::new_with_config_path(&args.rpc_config)?;
            let proof = provider.get_user_sub_tree_merkle_proof(args.checkpoint_id, args.root_level, args.leaf_level, args.leaf_index).await?;
            println!("{}", serde_json::to_string_pretty(&proof)?);
        }
        ...
        Commands::GetContractFunctionTreeLeafHash(args) => {
            ...
            use psy_data::traits::qdatastore::qtreedata::QTreeDataStoreReaderSync;
            let provider = RpcProvider::new_with_config_path(&args.rpc_config)?;
            let hash = provider.get_contract_function_tree_leaf_hash(args.checkpoint_id, args.contract_id, args.function_id).await?;
            println!("{}", serde_json::to_string_pretty(&hash)?);
        }
        Commands::GetContractFunctionTreeMerkleProof(args) => {
            ...
            use psy_data::traits::qdatastore::qtreedata::QTreeDataStoreReaderSync;
            let provider = RpcProvider::new_with_config_path(&args.rpc_config)?;
            let proof = provider.get_contract_function_tree_merkle_proof(args.checkpoint_id, args.contract_id, args.function_id).await?;
            println!("{}", serde_json::to_string_pretty(&proof)?);
        }
        ...
        Commands::GetCheckpointTreeLeafHash(args) => {
            ...
            use psy_data::traits::qdatastore::qtreedata::QTreeDataStoreReaderSync;
            let provider = RpcProvider::new_with_config_path(&args.rpc_config)?;
            let hash = provider.get_checkpoint_tree_leaf_hash(args.checkpoint_id, args.leaf_checkpoint_id).await?;
            println!("{}", serde_json::to_string_pretty(&hash)?);
        }
        Commands::GetCheckpointTreeMerkleProof(args) => {
            ...
            use psy_data::traits::qdatastore::qtreedata::QTreeDataStoreReaderSync;
            let provider = RpcProvider::new_with_config_path(&args.rpc_config)?;
            let proof = provider.get_checkpoint_tree_merkle_proof(args.checkpoint_id, args.leaf_checkpoint_id).await?;
            println!("{}", serde_json::to_string_pretty(&proof)?);
        }
