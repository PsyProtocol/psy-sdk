...
>
{
    fn get_first_user_for_public_key_hash_if_exists(
        store: &S,
        public_key: QEDHash,
    ) -> anyhow::Result<Option<QUserPublicKeyRecord>> {
        ...
        KVA::get_leq(
            store,
            &Hash4x64KeyWithId::new(public_key, max_user_id),
            user_id_bytes,
        )
    }
    fn get_users_for_public_key_hash_if_exists(
        ...
        let max_user_id = start_user_id
            .unwrap_or(real_max_user_id)
            .min(real_max_user_id);
        let user_id_bytes = ceil_div_usize(GLOBAL_USER_TREE_HEIGHT as usize, 8).min(8);

        Ok(KVA::get_fuzzy_range_leq_kv(
            store,
            &Hash4x64KeyWithId::new(public_key, max_user_id),
            user_id_bytes,
        )?
...
pub trait QEDUserPublicKeyHelperModelCore<
    ...
    KVA: KVQStoreAdapter<
        ...
        QUserPublicKeyRecord,
    >,
>: QEDUserPublicKeyHelperModelReaderCore<USER_PUBLIC_KEY_HELPER_TABLE_TYPE, S, KVA>
{
    fn delete_user_public_key_record(
        store: &mut S,
        user_id: u64,
        public_key: QEDHash,
    ) -> anyhow::Result<Option<QUserPublicKeyRecord>> {
    ...
    fn set_user_public_key_records(
        store: &S,
        records: &[QUserPublicKeyRecord],
    ) -> anyhow::Result<()> {
        let keys = records
            .iter()
...
pub struct QEDUserPublicKeyHelperModel<const USER_PUBLIC_KEY_HELPER_TABLE_TYPE: u16, S, KVA> {
    _store: S,
    _kva: KVA,
}
...
impl<
        ...
        >,
    > QEDUserPublicKeyHelperModelReaderCore<USER_PUBLIC_KEY_HELPER_TABLE_TYPE, S, KVA>
    for QEDUserPublicKeyHelperModel<USER_PUBLIC_KEY_HELPER_TABLE_TYPE, S, KVA>
{
}
impl<
        const USER_PUBLIC_KEY_HELPER_TABLE_TYPE: u16,
        ...
        KVA: KVQStoreAdapter<
            ...
            QUserPublicKeyRecord,
        >,
    > QEDUserPublicKeyHelperModelCore<USER_PUBLIC_KEY_HELPER_TABLE_TYPE, S, KVA>
    for QEDUserPublicKeyHelperModel<USER_PUBLIC_KEY_HELPER_TABLE_TYPE, S, KVA>
{
}
