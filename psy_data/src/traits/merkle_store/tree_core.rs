...
>: QEDMerkleTreeModelReaderCoreAsync<S, Hash, Hasher, TABLE_TYPE, MARK_LEAVES>
{
    ...
    async fn set_rehash_from_node_to_level_dmp_with_updates(
        store: &S,
        tree_height: usize,
        node: KVQMerkleNodeKey<TABLE_TYPE>,
        new_value: Hash,
        ...
        if root_level == node.level {
            ...
            Self::set_node(store, &node, &new_value).await?;
        ...
        let mut siblings_and_old = Self::get_nodes(store, tree_height, &siblings_old_value_root_keys).await?;
    ...
    async fn rehash_sub_tree(
        ...
        if sub_tree_height == 0 {
            ...
            Self::set_node(store, sub_root_key, &sub_root_value).await?;
            return Ok(());
        }
        ...
        for (i, x) in Self::get_nodes(store, tree_height, &child_keys).await?.chunks_exact(2).enumerate() {
