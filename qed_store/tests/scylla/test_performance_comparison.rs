...
async fn benchmark_insert(name: &str, count: usize, insert_fn: impl Fn(usize) -> Result<()>) -> Result<f64> {
    ...
    println!("{} - Insert {} items: {:.3}s ({:.0} ops/sec)", name, count, elapsed, count as f64 / elapsed);
    Ok(elapsed)
}
    ...
    for batch_size in batch_sizes {
        ...
        for batch_idx in 0..num_batches {
            ...
            let batch_items: Vec<_> = (start_idx..end_idx)
                .map(|i| KVQPair { key: &keys[i], value: &values[i] })
                .collect();
            store.set_many_ref(&batch_items).await?;
        }
        let insert_elapsed = start.elapsed().as_secs_f64();
        println!("{} - Batch insert (size {}): {:.3}s ({:.0} ops/sec)", 
            name, batch_size, insert_elapsed, count as f64 / insert_elapsed);
        ...
        println!("{} - Batch read (size {}): {:.3}s ({:.0} ops/sec)", 
            name, batch_size, read_elapsed, count as f64 / read_elapsed);
...
async fn test_performance_fuzzy_search() -> Result<()> {
    ...
    let keys: Vec<Vec<u8>> = (0..count).map(|i| {
        format!("prefix{:04}suffix{:04}", i / 100, i).into_bytes()
    }).collect();
    let values: Vec<Vec<u8>> = (0..count).map(|i| format!("value{:08}", i).into_bytes()).collect();
    ...
    let query_keys: Vec<Vec<u8>> = (0..1000).map(|i| {
        format!("prefix{:04}suffix{:04}", i / 100, i * 5 + 2).into_bytes()
    }).collect();
    ...
    for thread_id in 0..thread_count {
        ...
        let handle = tokio::spawn(async move {
            let keys: Vec<Vec<u8>> = (0..ops_per_thread).map(|i| {
                format!("thread{:02}key{:05}", thread_id, i).into_bytes()
            }).collect();
