...
use crate::models::{
    ContractResponse, ContractSummary, ListContractsParams, ListContractsResponse
};
...
async fn get_contract_by_id_handler(
...
/// List contracts with optional filtering by deployer, checkpoint, or function name
async fn list_contracts_handler(
    ...
    if let Some(function_name) = params.function_name {
        match ContractRepository::search_by_function_name(
            &service.pool,
            &function_name,
            limit,
            offset
            ...
            Ok(contracts) => {
                ...
                let summaries: Vec<ContractSummary> = contracts.into_iter().map(|c| {
                    ContractSummary {
    ...
    let contracts = match ContractRepository::list(
        &service.pool,
        limit,
        offset,
    ...
    let total = match ContractRepository::count(
        &service.pool,
        params.deployer.as_deref(),
        params.checkpoint_id,
    ).await {
...
async fn get_all_function_names_handler(
    State(service): State<ApiService>,
