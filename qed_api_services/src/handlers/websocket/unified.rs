...
use crate::models::{UserEvent, WorkerEvent, TpsData};
...
impl UnifiedWebSocketManager {
    ...
    async fn is_connection_alive(&self, conn: &UnifiedWebSocketConnection) -> bool {
        ...
        if elapsed.num_seconds() > PONG_TIMEOUT_SECS {
            warn!(
                "Connection {} timed out: no pong received in {} seconds",
                conn.id,
                elapsed.num_seconds()
            );
            false
...
pub async fn unified_websocket_handler(
    ws: WebSocketUpgrade,
...
async fn handle_unified_websocket(
    socket: WebSocket,
    service: ApiService,
    initial_channels: Vec<Channel>,
    initial_filters: ChannelFilters,
) {
...
async fn handle_client_message(
    ...
    match message {
        ClientMessage::Subscribe { channels } => {
            match manager.update_subscriptions(connection_id, channels.clone(), true).await {
                Ok(changed) => {
                    if !changed.is_empty() {
        ...
        ClientMessage::Unsubscribe { channels } => {
            match manager.update_subscriptions(connection_id, channels.clone(), false).await {
                Ok(changed) => {
                    if !changed.is_empty() {
                        let connections = manager.connections.read().await;
                        if let Some(conn) = connections.get(&connection_id) {
                            let _ = conn.sender.send(ServerMessage::Unsubscribed {
                                channels: changed,
                            });
                        }
                    }
        ...
        ClientMessage::UpdateFilters { filters } => {
            match manager.update_filters(connection_id, filters).await {
                Ok(()) => {
