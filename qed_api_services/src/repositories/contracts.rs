...
use crate::models::{
    Contract, ContractResponse, ContractSummary, ContractMetadataReport,
    QFunctionMetadata, UserContractMetadata
};
...
impl ContractRepository {
    ...
    pub async fn upsert_from_report(
        pool: &PgPool,
    ...
    pub async fn get_by_id(
        pool: &PgPool,
        contract_id: i64,
    ) -> anyhow::Result<Option<ContractResponse>> {
        let contract = sqlx::query_as::<_, Contract>(
            r#"
    ...
    pub async fn list(
        ...
        let summaries: Result<Vec<ContractSummary>, anyhow::Error> = contracts
            .into_iter()
            .map(|c| Self::contract_to_summary(c))
            .collect();
    ...
    pub async fn search_by_function_name(
        ...
        limit: i64,
        offset: i64,
    ) -> anyhow::Result<Vec<ContractResponse>> {
    ...
    fn contract_to_response(contract: Contract) -> anyhow::Result<ContractResponse> {
        ...
        let user_metadata: Option<UserContractMetadata> = contract.metadata
            .as_object()
        ...
        let (state_tree_height, function_count, functions) = if let Some(meta) = user_metadata {
            (
                Some(meta.state_tree_height),
                Some(meta.function_count),
                meta.functions,
            )
        } else {
            // Fallback: try to extract just the functions array
            let functions = Self::extract_functions_from_metadata(&contract.metadata)
                .unwrap_or_default();
            (None, None, functions)
        };
    ...
    fn contract_to_summary(contract: Contract) -> anyhow::Result<ContractSummary> {
        ...
        let function_count = contract.metadata
            .get("function_count")
            .and_then(|v| v.as_u64())
            .map(|v| v as usize)
            .or_else(|| {
    ...
    fn extract_functions_from_metadata(
        metadata: &serde_json::Value,
    ) -> anyhow::Result<Vec<QFunctionMetadata>> {
