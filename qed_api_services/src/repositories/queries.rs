...
impl UserInfo {
    ...
    pub async fn find_by_public_key(pool: &PgPool, public_key: &str) -> Result<Option<Self>> {
        let row = sqlx::query!(
            r#"
...
impl WorkerEvent {
    ...
    pub async fn create(
        ...
        let parsed_job_id = job_id_from_json(row.job_id).unwrap_or(default_job_id);
