...
fn render_messages(f: &mut Frame, area: Rect, app: &App) {
    ...
    let messages_widget = List::new(messages)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(format!(" Messages ({}) ", app.logs.len()))
        )
...
fn handle_server_message(msg: ServerMessage, app: &mut App) {
    match msg {
            ...
            match channel.as_str() {
                "worker_events" => {
                    ...
                    let status = data.get("status")
                        .and_then(|v| v.as_str())
                        .unwrap_or("unknown");
                    app.add_log(LogMessage::Received(
                        format!("WORKER EVENT: status={}", status)
                    ));
                }
                "user_events" => {
                    ...
                    let user_id = data.get("user_id")
                        ...
                        .unwrap_or("unknown");
                    app.add_log(LogMessage::Received(
                        format!("USER EVENT: user_id={}", user_id)
                    ));
                }
                "tps" => {
                    ...
                    let tps = data.get("tps")
                        ...
                        .unwrap_or(0.0);
                    app.add_log(LogMessage::Received(
                        format!("TPS UPDATE: {:.2}", tps)
                    ));
                }
                _ => {
                    app.add_log(LogMessage::Received(
                        format!("EVENT: channel={}", channel)
                    ));
                }
            }
        }
        ServerMessage::Subscribed { channels } => {
            app.add_log(LogMessage::Received(
                format!("Subscribed to: {:?}", channels)
            ));
        }
        ServerMessage::Unsubscribed { channels } => {
            app.add_log(LogMessage::Received(
                format!("Unsubscribed from: {:?}", channels)
            ));
        }
        ServerMessage::Error { code, message } => {
            app.stats.errors += 1;
            app.add_log(LogMessage::Error(
                format!("[{}] {}", code, message)
            ));
        }
        ServerMessage::Ping => {
