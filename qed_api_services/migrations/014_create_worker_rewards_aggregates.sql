...
-- Create continuous aggregates based on checkpoint_reward_distributions
-- These provide time-bucketed views of rewards, while worker_rewards table (006) provides all-time totals
...
BEGIN
    ...
    -- This queries checkpoint_reward_distributions which has ACTUAL calculated rewards
    sql_query := format(
...
-- These aggregate actual rewards from checkpoint_reward_distributions
SELECT create_worker_rewards_aggregate(
               'worker_rewards_1d',
               '1 day',
               '7 days',
               '1 hour',
...
COMMENT ON VIEW worker_rewards_1d IS
'Daily aggregation of worker rewards from checkpoint_reward_distributions. Shows actual calculated rewards per worker per day.';

COMMENT ON VIEW worker_rewards_1w IS
'Weekly aggregation of worker rewards from checkpoint_reward_distributions. Shows actual calculated rewards per worker per week.';

COMMENT ON VIEW worker_rewards_1m IS
'Monthly aggregation of worker rewards from checkpoint_reward_distributions. Shows actual calculated rewards per worker per month.';