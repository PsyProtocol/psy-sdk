...
use crate::request::{
    Id, QAddWithdrawalRPCRequest, QClaimDepositRPCRequest, QDeployContractRPCRequest, QRegisterUserRPCRequest,
    QSubmitEndCapRPCRequest, QTokenTransferRPCRequest,
    RequestParams, ResponseResult, RpcRequest, RpcResponse, Version,
};
...
impl QUserRpcProvider for RpcProvider {
    ...
    async fn add_withdrawal<F: RichField>(&self, _req: QAddWithdrawalRPCRequest) -> anyhow::Result<()> {
        unimplemented!()
    }

    async fn claim_deposit<F: RichField>(&self, _req: QClaimDepositRPCRequest) -> anyhow::Result<()> {
        unimplemented!()
    }

    async fn token_transfer<F: RichField>(&self, _req: QTokenTransferRPCRequest) -> anyhow::Result<()> {
        unimplemented!()
    }
    ...
    async fn submit_end_cap_proof<F: RichField>(
        ...
        qed_rpc_call!(self, rpc_url, RequestParams::<F>::SubmitEndCap(req)).await
    }
}

impl RpcProvider {
    pub async fn get_user_id<F: RichField>(&self, public_key_param: QHashOut<F>) -> anyhow::Result<u64> {
        let coordinator_url = self.get_coordinator_url()?;
        let response = qed_rpc_call_back!(
            self,
            coordinator_url,
            RequestParams::<F>::GetUserId(public_key_param),
            u64
        ).await.map_err(|e| anyhow::format_err!("HTTP request failed: {}", e))?;
        match response.result {
