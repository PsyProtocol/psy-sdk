                        ...
                        TxnManagerMessage::Commit { tx, sender } => {
                            sender
                                .send({
                                    ...
                                    mdbx_result(unsafe { ffi::mdbx_txn_commit_ex(tx.0, latency.mdb_commit_latency()) }).map(|v| (v, latency))
                                })
                                .unwrap();
                        }
                    },
...
mod read_transactions {
    ...
    pub(super) struct ReadTransactions {
        ...
        /// stored as a value.
        ///
        /// The backtrace of the transaction opening is recorded only when debug
    ...
    impl ReadTransactions {
        ...
        pub(super) fn add_active(&self, ptr: *mut ffi::MDBX_txn, tx: TransactionPtr) {
            let _ = self.active.insert(
                ptr as usize,
                (tx, Instant::now(), cfg!(debug_assertions).then(|| Arc::new(Backtrace::force_capture()))),
            );
        }
        ...
        pub(super) fn start_monitor(self: Arc<Self>) {
            let task = move || {
                ...
                loop {
                        ...
                        if duration > self.max_duration {
                            ...
                            match result {
                                Ok((txn_ptr, duration, error)) => {
                                    ...
                                    // iterate through it.
                                    timed_out_active.push((txn_ptr, duration, backtrace.clone(), error));
                                }
                                Err(err) => {
                    ...
                    let sleep_duration =
                        READ_TRANSACTIONS_CHECK_INTERVAL.min(self.max_duration - max_active_transaction_duration.unwrap_or_default());
