...
use psy_data::config::store_config::{
    CHECKPOINT_BLOCK_STATE_TABLE_TYPE, CHECKPOINT_HASH_HELPER_TABLE_TYPE, CHECKPOINT_LEAF_TABLE_TYPE, CHECKPOINT_SYNC_INFO_TABLE_TYPE,
    CHECKPOINT_TREE_TABLE_TYPE, CONTRACT_CODE_TABLE_TYPE, CONTRACT_FUNCTION_TREE_TABLE_TYPE, CONTRACT_LEAF_TABLE_TYPE, CONTRACT_TREE_TABLE_TYPE,
    DEPOSIT_TREE_TABLE_TYPE, REALM_STATUS_TABLE_TYPE, STAGING_CHECKPOINT_INFO_TABLE_TYPE, STAGING_DELTA_RECORD_TABLE_TYPE,
    USER_CONTRACT_STATE_TREE_TABLE_TYPE, USER_CONTRACT_TREE_TABLE_TYPE, USER_LEAF_TABLE_TYPE, USER_PUBLIC_KEY_HELPER_TABLE_TYPE,
    USER_REGISTRATION_TREE_TABLE_TYPE, USER_TREE_TABLE_TYPE, WITHDRAWAL_TREE_TABLE_TYPE,
};
use scylla::{Session, SessionBuilder};

use super::{clustering_store::ScyllaClusteringStore, config::ScyllaDBConfig, kvq_store::ScyllaKVQStore};
...
impl ScyllaStore {
    ...
    pub async fn new_with_config(uri: &str, keyspace: &str, config: Option<ScyllaDBConfig>) -> Result<Self> {
    ...
    async fn register_all_tables(&mut self, keyspace: &str) -> Result<()> {
        self.register_clustering_table(CHECKPOINT_TREE_TABLE_TYPE, keyspace, "checkpoint_trees", 8)
            .await?;
        self.register_clustering_table(USER_TREE_TABLE_TYPE, keyspace, "user_trees", 8).await?;
        self.register_clustering_table(CONTRACT_TREE_TABLE_TYPE, keyspace, "contract_trees", 8)
            .await?;
        self.register_clustering_table(CONTRACT_FUNCTION_TREE_TABLE_TYPE, keyspace, "contract_function_trees", 8)
            .await?;
        ...
        self.register_clustering_table(WITHDRAWAL_TREE_TABLE_TYPE, keyspace, "withdrawal_trees", 8)
            .await?;
        self.register_clustering_table(USER_REGISTRATION_TREE_TABLE_TYPE, keyspace, "user_registration_trees", 8)
            .await?;
        self.register_clustering_table(USER_CONTRACT_TREE_TABLE_TYPE, keyspace, "user_contract_trees", 8)
            .await?;
        self.register_clustering_table(USER_CONTRACT_STATE_TREE_TABLE_TYPE, keyspace, "user_contract_state_trees", 8)
            .await?;

        self.register_clustering_table(USER_LEAF_TABLE_TYPE, keyspace, "user_leaves", 4).await?;

        self.register_clustering_table(CHECKPOINT_LEAF_TABLE_TYPE, keyspace, "checkpoint_leaves", 8)
            .await?;

        self.register_clustering_table(CHECKPOINT_BLOCK_STATE_TABLE_TYPE, keyspace, "checkpoint_block_states", 8)
            .await?;
        ...
        self.register_clustering_table(CHECKPOINT_SYNC_INFO_TABLE_TYPE, keyspace, "checkpoint_sync_info", 8)
            .await?;

        self.register_kvq_table(CHECKPOINT_HASH_HELPER_TABLE_TYPE, keyspace, "checkpoint_hash_helpers")
            .await?;

        self.register_kvq_table(USER_PUBLIC_KEY_HELPER_TABLE_TYPE, keyspace, "user_public_key_helpers")
            .await?;

        self.register_kvq_table(STAGING_CHECKPOINT_INFO_TABLE_TYPE, keyspace, "staging_checkpoint_info")
            .await?;

        self.register_kvq_table(STAGING_DELTA_RECORD_TABLE_TYPE, keyspace, "staging_delta_records")
            .await?;

        self.register_kvq_table(REALM_STATUS_TABLE_TYPE, keyspace, "realm_status").await?;
    ...
    async fn register_kvq_table(&mut self, table_type: u16, keyspace: &str, table_name: &str) -> Result<()> {
        ...
        let store = ScyllaKVQStore::new_with_session(self.session.clone(), keyspace, table_name).await?;

        self.stores[table_type as usize] = Some(Arc::new(store));
        Ok(())
    }

    async fn register_clustering_table(&mut self, table_type: u16, keyspace: &str, table_name: &str, clustering_key_size: usize) -> Result<()> {
        if table_type as usize >= 50 {
            return Err(anyhow::anyhow!("Table type {} exceeds maximum of 49", table_type));
        }

        let store = ScyllaClusteringStore::new_with_session(self.session.clone(), keyspace, table_name, clustering_key_size).await?;
    ...
    fn set_and_delete_many(&self, keys_to_set: &[KVQPair<&Vec<u8>, &Vec<u8>>], keys_to_delete: &[Vec<u8>]) -> Result<()> {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current()
                .block_on(async { <Self as KVQBinaryStoreAsync>::set_and_delete_many(self, keys_to_set, keys_to_delete).await })
        })
    }
}
