...
fn test_kvq_simple_store_arc() -> anyhow::Result<()> {
    ...
    let lf1 = UserTreeStore::<Arc<KVQSimpleMemoryBackingStore>>::set_leaf_fc(&st, 0, 0, QHashOut::from_values(1, 2, 3, 4))?;
    println!("lf2 result: {}", lf1.verify::<PoseidonHash>());

    let lf2 = UserTreeStore::<Arc<KVQSimpleMemoryBackingStore>>::set_leaf_fc(&st, 1, 1, QHashOut::from_values(5, 6, 7, 8))?;
    ...
    let lf = UserTreeStore::<Arc<KVQSimpleMemoryBackingStore>>::get_leaf_fc(&st, 2, 0)?;
...
fn test_kvq_simple_store() -> anyhow::Result<()> {
    ...
    let lf1 = UserTreeStore::<KVQSimpleMemoryBackingStore>::set_leaf_fc(&st, 0, 0, QHashOut::from_values(1, 2, 3, 4))?;
    println!("lf2 result: {}", lf1.verify::<PoseidonHash>());

    let lf2 = UserTreeStore::<KVQSimpleMemoryBackingStore>::set_leaf_fc(&st, 1, 1, QHashOut::from_values(5, 6, 7, 8))?;
    println!("lf2 result: {}", lf2.verify::<PoseidonHash>());
    t.lap("start modifications");

    for i in 0..100000 {
        let _tmp = UserTreeStore::<KVQSimpleMemoryBackingStore>::set_leaf_fc(&st, 2, i, QHashOut::from_values(i + 5, 6, 7, 8))?;
    }
    t.batch_average("end modifications", "set_user_leaf", 100000);

    let lf = UserTreeStore::<KVQSimpleMemoryBackingStore>::get_leaf_fc(&st, 2, 0)?;
