...
async fn test_get_leq_clustering_tables() -> Result<()> {
    ...
    // [table_type (2 bytes) + tree_id (8 bytes) + checkpoint_id (8 bytes) + suffix
    // (4 bytes)] = 22 bytes
    ...
    for (query_id, expected_id) in query_checkpoint_ids.iter().zip(expected_results.iter()) {
        ...
        println!("   Query checkpoint {} - LibMDBX: {:?}, ScyllaDB: {:?}", query_id, mdbx_str, scylla_str);
...
async fn test_checkpoint_block_state_special_case() -> Result<()> {
    ...
    for (query_id, expected_id) in test_queries {
        ...
        println!("   Query {} - LibMDBX: {:?}, ScyllaDB: {:?}", query_id, mdbx_str, scylla_str);

        // For this special case, both should return the same result
        assert_eq!(mdbx_result, scylla_result, "Checkpoint block state get_leq should be consistent");
