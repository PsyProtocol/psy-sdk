...
async fn test_get_leq_consistency() -> Result<()> {
    ...
    // Keys might be structured as [table_type (2 bytes) + user_id (8 bytes) +
    // version (4 bytes)]
    ...
    for (query_version, expected_version) in test_queries {
        ...
        println!("   Query version {} - LibMDBX: {:?}, ScyllaDB: {:?}", query_version, mdbx_str, scylla_str);

        assert_eq!(mdbx_result, scylla_result, "Fuzzy get_leq mismatch for query version {}", query_version);
...
async fn test_get_fuzzy_range_leq_kv_consistency() -> Result<()> {
    ...
    let test_keys = vec![
        (
            vec![
                0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x05,
            ],
            6,
        ), // Should return entries 0-5
        (
            vec![
                0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x09,
            ],
            10,
    ...
    for (query_key, expected_count) in test_keys {
        ...
        assert_eq!(mdbx_sorted.len(), scylla_sorted.len(), "Different number of results returned");

        // Compare each result
        for (i, (mdbx_kv, scylla_kv)) in mdbx_sorted.iter().zip(scylla_sorted.iter()).enumerate() {
            println!(
                "   Entry {} - Key match: {}, Value match: {}",
                i,
        ...
        assert_eq!(mdbx_sorted.len(), expected_count, "Unexpected number of results for query");
    }

    println!("\n4. Testing edge cases");

    // Test with non-existent prefix
    let non_existent_key = vec![
    ...
    println!(
        "   Non-existent prefix - LibMDBX: {} entries, ScyllaDB: {} entries",
        mdbx_empty.len(),
        scylla_empty.len()
    );
...
async fn test_checkpoint_tree_fuzzy_operations() -> Result<()> {
    ...
    for height in &checkpoint_heights {
        // Key structure: [table_type (2 bytes) + tree_id (8 bytes) + height (8 bytes) +
        // node_id (4 bytes)]
        let mut key = table_type.to_be_bytes().to_vec();
    ...
    for query_height in query_heights {
        ...
        println!("   Query height {} - LibMDBX: {:?}, ScyllaDB: {:?}", query_height, mdbx_str, scylla_str);
        ...
        assert_eq!(mdbx_result, scylla_result, "Checkpoint get_leq mismatch for height {}", query_height);
    }
