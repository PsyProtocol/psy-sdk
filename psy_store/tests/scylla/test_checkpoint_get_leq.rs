...
async fn test_checkpoint_block_state_get_latest() -> anyhow::Result<()> {
    ...
    type BlockStateAdapter = KVQStandardAdapter<ScyllaKVQStore, BlockStateKey, QEDL2BlockState>;

    // Insert some test block states
    let block_state_0 = QEDL2BlockState {
        checkpoint_id: 0,
    ...
    let block_state_1 = QEDL2BlockState {
        checkpoint_id: 1,
        next_add_withdrawal_id: 0,
        next_process_withdrawal_id: 0,
        next_deposit_id: 0,
    ...
    let block_state_5 = QEDL2BlockState {
        checkpoint_id: 5,
