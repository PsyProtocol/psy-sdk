...
use psy_store::store::scylla::{clustering_store::ScyllaClusteringStore, kvq_store::ScyllaKVQStore};
...
async fn test_kvq_store_get_many_leq_chunked() -> Result<()> {
    ...
    let base_keys: Vec<Vec<u8>> = (0..50)
        .map(|i| {
            format!("key{:06}", i * 2).into_bytes() // Store even numbers: 0, 2,
                                                    // 4, ..., 98
        })
        .collect();
    let values: Vec<Vec<u8>> = (0..50).map(|i| format!("value{:03}", i).into_bytes()).collect();

    let items: Vec<_> = base_keys.iter().zip(values.iter()).map(|(k, v)| KVQPair { key: k, value: v }).collect();
    store.set_many_ref(&items).await?;

    // Query for odd numbers, which don't exist, so should return the previous even
    // number
    let query_keys: Vec<Vec<u8>> = (0..50)
        .map(|i| {
            format!("key{:06}", i * 2 + 1).into_bytes() // Query odd numbers: 1,
                                                        // 3, 5, ..., 99
        })
        .collect();
    ...
    for (i, result) in results.iter().enumerate() {
        ...
        assert_eq!(result.as_ref().unwrap(), &values[i], "Failed at index {}: expected value{:03}", i, i);
    }
...
async fn test_clustering_store_get_many_leq_chunked() -> Result<()> {
    ...
    let query_keys: Vec<Vec<u8>> = (0..32u64)
        .map(|i| {
            let partition_id = i / 8;
...
async fn test_concurrent_batch_stress() -> Result<()> {
    ...
    for thread_id in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            let keys: Vec<Vec<u8>> = (0..30).map(|i| format!("thread{:02}key{:03}", thread_id, i).into_bytes()).collect();
            let values: Vec<Vec<u8>> = (0..30).map(|i| format!("thread{:02}value{:03}", thread_id, i).into_bytes()).collect();
