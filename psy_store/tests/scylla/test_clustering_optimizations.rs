...
use psy_store::store::scylla::clustering_store::ScyllaClusteringStore;
...
async fn test_clustering_get_leq_with_clustering_key_size() -> Result<()> {
    ...
    let store = ScyllaClusteringStore::new(&config.uri, &config.keyspace, "test_clustering_leq_opt", clustering_key_size).await?;
...
async fn test_clustering_get_fuzzy_range_performance() -> Result<()> {
    let config = TestConfig::new().await?;
    let clustering_key_size = 8;
    let store = ScyllaClusteringStore::new(&config.uri, &config.keyspace, "test_clustering_range_perf", clustering_key_size).await?;
...
async fn test_clustering_batch_operations_performance() -> Result<()> {
    let config = TestConfig::new().await?;
    let clustering_key_size = 8;
    let store = ScyllaClusteringStore::new(&config.uri, &config.keyspace, "test_clustering_batch_perf", clustering_key_size).await?;
...
async fn test_clustering_concurrent_batch_operations() -> Result<()> {
    let config = TestConfig::new().await?;
    let clustering_key_size = 8;
    let store = Arc::new(ScyllaClusteringStore::new(&config.uri, &config.keyspace, "test_clustering_concurrent", clustering_key_size).await?);
...
async fn test_clustering_get_many_leq_chunked() -> Result<()> {
    let config = TestConfig::new().await?;
    let clustering_key_size = 8;
    let store = ScyllaClusteringStore::new(&config.uri, &config.keyspace, "test_clustering_many_leq", clustering_key_size).await?;
