...
async fn test_performance_comparison_detailed() -> Result<()> {
    ...
    for size in test_sizes {
        ...
        println!(
            "ScyllaDB Batch Insert: {:.3}s ({:.0} ops/sec)",
            scylla_batch_insert,
            size as f64 / scylla_batch_insert
        );
        ...
        println!(
            "ScyllaDB Batch Read: {:.3}s ({:.0} ops/sec)",
            scylla_batch_read,
            size as f64 / scylla_batch_read
        );
        ...
        println!(
            "- Insert: ScyllaDB is {:.1}x {} than MDBX",
            if scylla_batch_insert < mdbx_insert {
                mdbx_insert / scylla_batch_insert
            } else {
                scylla_batch_insert / mdbx_insert
            },
            if scylla_batch_insert < mdbx_insert { "faster" } else { "slower" }
        );
        println!(
            "- Read: ScyllaDB is {:.1}x {} than MDBX",
            if scylla_batch_read < mdbx_read {
                mdbx_read / scylla_batch_read
            } else {
                scylla_batch_read / mdbx_read
            },
...
async fn test_get_leq_performance_comparison() -> Result<()> {
    ...
    // Insert test data
    let count = 10000;
    let keys: Vec<Vec<u8>> = (0..count).step_by(2).map(|i| format!("key{:08}", i).into_bytes()).collect();
    let values: Vec<Vec<u8>> = (0..count / 2).map(|i| format!("value{:08}", i).into_bytes()).collect();
    ...
    let query_keys: Vec<Vec<u8>> = (0..query_count)
        .map(|i| {
    ...
    println!(
        "ScyllaDB get_many_leq: {:.3}s ({:.0} ops/sec)",
        scylla_time,
        query_count as f64 / scylla_time
    );
    ...
    println!(
        ...
        if scylla_time < mdbx_time {
            mdbx_time / scylla_time
        } else {
            scylla_time / mdbx_time
        },
    ...
    for thread_count in thread_counts {
        ...
        for thread_id in 0..thread_count {
            ...
            let handle = tokio::spawn(async move {
                let keys: Vec<Vec<u8>> = (0..ops_per_thread).map(|i| format!("t{:02}k{:05}", thread_id, i).into_bytes()).collect();
