...
async fn test_debug_fuzzy_comparison() -> Result<()> {
    ...
    println!(
        "    key1 suffix: {:?} = {}",
        &key1[2..],
        u64::from_be_bytes(key1[2..].try_into().unwrap())
    );
    println!(
        "    key2 suffix: {:?} = {}",
        &key2[2..],
        u64::from_be_bytes(key2[2..].try_into().unwrap())
    );
    println!(
        "    query suffix: {:?} = {}",
        &query[2..],
        u64::from_be_bytes(query[2..].try_into().unwrap())
    );

    let result = <ScyllaStore as KVQBinaryStoreAsync>::get_leq(&scylla_store, &query, 8).await?;
