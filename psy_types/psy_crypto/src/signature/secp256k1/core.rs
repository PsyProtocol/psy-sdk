use k256::elliptic_curve::{point::DecompressPoint, sec1::ToEncodedPoint};
use plonky2::{
...
use crate::signature::secp256k1::curve::{
...
pub fn secp256k1_scalar_from_bytes(bytes: &[u8], offset: usize) -> Secp256K1Scalar {
    ...
    */
    ...
    let arr: [u64; 4] = core::array::from_fn(|i| u64::from_be_bytes(bytes[(3 - i) * 8 + offset..(3 - i + 1) * 8 + offset].try_into().unwrap()));
    Secp256K1Scalar(arr)
}
...
impl PsyCompressedSecp256K1Signature {
    pub fn to_btc_script(&self) -> Vec<u8> {
        ...
        let sig_stack_raw = [vec![combined_rs_length + 3, 0x30u8, combined_rs_length], r, s, vec![0x01]].concat();
...
impl<F: RichField> TryFrom<&PsyCompressedSecp256K1Signature> for PsyPreparedSecp256K1Signature<F> {
    ...
    fn try_from(value: &PsyCompressedSecp256K1Signature) -> Result<Self, Self::Error> {
        ...
        let public_key_point = k256::AffinePoint::decompress(value.public_key[1..33].into(), (value.public_key[0] & 0x1u8).into());
...
pub fn hash256_to_hashout_u224<F: RichField>(hash: Hash256) -> HashOut<F> {
    HashOut {
        elements: core::array::from_fn(|i| {
            F::from_canonical_u64(u64::from_le_bytes(hash.0[i * 8..(i + 1) * 8].try_into().unwrap()) & 0x00FFFFFFFFFFFFFF)
        }),
    }
}
