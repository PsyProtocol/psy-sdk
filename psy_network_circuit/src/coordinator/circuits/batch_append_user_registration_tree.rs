...
use psy_common_circuit::{
    builder::{comparison::CircuitBuilderComparison, hash::core::CircuitBuilderHashCore, pad_circuit::CircuitBuilderQEDCommonGates},
...
use crate::{
    coordinator::gadgets::append_user_registration_tree::BatchAppendUserRegistrationTreeGadget,
...
impl<C: GenericConfig<D>, const D: usize> QStandardCircuit<C, D> for BatchAppendUserRegistrationTreeCircuit<C, D>
where
...
impl<S: QProofStoreReaderAsync + Send + Sync, L: CircuitInfoLibrary<C, D> + Send + Sync, C: GenericConfig<D> + 'static, const D: usize>
    QStandardCircuitProvableWithProofStoreAndRefLibraryAsync<S, L, C, D> for BatchAppendUserRegistrationTreeCircuit<C, D>
where
    C::Hasher: AlgebraicHasher<C::F> + MerkleZeroHasher<HashOut<C::F>>,
{
