...
impl UPSEndCapResultCompactGadget {
    ...
    pub fn set_witness<F: RichField>(&self, witness: &mut impl Witness<F>, target: &UPSEndCapResultCompact<F>) -> anyhow::Result<()> {
        ...
        witness.set_hash_target(self.checkpoint_tree_root_hash, target.checkpoint_tree_root_hash.0)?;
    ...
    pub fn to_hash<H: AlgebraicHasher<F>, F: RichField + Extendable<D>, const D: usize>(&self, builder: &mut CircuitBuilder<F, D>) -> HashOutTarget {
        ...
        let end_cap_result_hash = builder.hash_two_to_one::<H>(self.checkpoint_tree_root_hash, user_leaf_change_combo_with_user_id);
