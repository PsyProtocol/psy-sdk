...
mod tests {
    ...
    use psy_common_circuit::{
        circuits::{
            lookalikes::{
                get_agg_state_transition_type_d_lookalike_circuit_data, get_end_cap_type_e_lookalike_circuit_data,
                get_guta_type_c_lookalike_circuit_data,
            },
            traits::qstandard::QStandardCircuit,
        },
        treeprover::{
            aggregation::{state_transition::AggStateTransitionCircuit, state_transition_dummy::AggStateTransitionDummyCircuit},
            qrecursion::standard::manager::portable::circuits::PortableQTreeRecursionCircuits,
            traits::TreeProverAggCircuit,
        },
    };
    use psy_core::{
        config::network_constants::{QED_NETWORK_MAGIC_REGTEST, UPS_CIRCUIT_WHITELIST_TREE_HEIGHT, UPS_SESSION_PROOF_TREE_HEIGHT},
    ...
    fn check_guta_lookalike() {
        ...
        let two_guta = GUTAVerifyTwoEndCapCircuit::<C, D>::new(
            ...
            end_cap_lookalike.verifier_only.constants_sigmas_cap.height(),
            QHashOut::rand(),
        );
    ...
    fn check_agg_state_transition_lookalike() {
        ...
        let agg_state_transition_lookalike = get_agg_state_transition_type_d_lookalike_circuit_data::<C, D>();
        ...
        let agg_state = AggStateTransitionCircuit::<C, D>::new(
            ...
            agg_state_dummy.get_verifier_config_ref().constants_sigmas_cap.height(),
        );
