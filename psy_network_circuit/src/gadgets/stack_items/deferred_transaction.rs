...
use psy_common_circuit::traits::{AlgebraicHashableTarget, CreatableTarget, WitnessValueFor};
...
pub struct DeferredTransactionStackItemGadget {
    pub call_data: DPNProvingSessionCompactMethodCallGadget,
}

impl DeferredTransactionStackItemGadget {
    fn add_virtual_to<F: RichField + Extendable<D>, const D: usize>(builder: &mut CircuitBuilder<F, D>) -> Self {
...
impl CreatableTarget for DeferredTransactionStackItemGadget {
...
impl AlgebraicHashableTarget for DeferredTransactionStackItemGadget {
...
impl<F: RichField> WitnessValueFor<DeferredTransactionStackItemGadget, F, true> for DPNProvingSessionCompactMethodCall<F> {
    fn set_for_witness(&self, witness: &mut impl Witness<F>, target: &DeferredTransactionStackItemGadget) -> anyhow::Result<()> {
        target.set_witness(witness, self)
    }
}

impl<F: RichField> WitnessValueFor<DeferredTransactionStackItemGadget, F, false> for DPNProvingSessionCompactMethodCall<F> {
    fn set_for_witness(&self, witness: &mut impl Witness<F>, target: &DeferredTransactionStackItemGadget) -> anyhow::Result<()> {
        target.set_witness(witness, self)
    }
}
