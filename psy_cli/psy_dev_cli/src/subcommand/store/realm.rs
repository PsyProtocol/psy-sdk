...
use psy_node::realm::error::RpcError;
use psy_store::{node::realm::PsyRealmStoreReaderAsync, store::journal::Journal};
...
impl RealmStoreRpcServer for StoreProvider {
    ...
    async fn get_checkpoint_leaf_data(&self, checkpoint_id: u64) -> RpcResult<PsyCheckpointLeaf<F>> {
        Ok(PsyRealmStoreReaderAsync::get_checkpoint_leaf_data(&self.store, checkpoint_id)
            .await
            .map_err(RpcError::Anyhow)?)
    }

    async fn get_latest_block_state(&self) -> RpcResult<PsyBlockState> {
        Ok(PsyRealmStoreReaderAsync::get_latest_block_state(&self.store)
            .await
            .map_err(RpcError::Anyhow)?)
    }

    async fn get_block_state(&self, checkpoint_id: u64) -> RpcResult<PsyBlockState> {
        Ok(PsyRealmStoreReaderAsync::get_block_state(&self.store, checkpoint_id)
            .await
            .map_err(RpcError::Anyhow)?)
    }

    async fn get_user_registration_tree_root(&self, checkpoint_id: u64) -> RpcResult<QHashOut<F>> {
        Ok(PsyRealmStoreReaderAsync::get_user_registration_tree_root(&self.store, checkpoint_id)
            .await
            .map_err(RpcError::Anyhow)?)
    }

    async fn get_latest_checkpoint_tree_root(&self) -> RpcResult<QHashOut<F>> {
        Ok(PsyRealmStoreReaderAsync::get_latest_checkpoint_tree_root(&self.store)
            .await
            .map_err(RpcError::Anyhow)?)
    }

    async fn get_checkpoint_tree_root(&self, checkpoint_id: u64) -> RpcResult<QHashOut<F>> {
        Ok(PsyRealmStoreReaderAsync::get_checkpoint_tree_root(&self.store, checkpoint_id)
            .await
            .map_err(RpcError::Anyhow)?)
    }
    ...
    async fn get_checkpoint_tree_leaf_hash(&self, checkpoint_id: u64, leaf_checkpoint_id: u64) -> RpcResult<QHashOut<F>> {
        Ok(
            PsyRealmStoreReaderAsync::get_checkpoint_tree_leaf_hash(&self.store, checkpoint_id, leaf_checkpoint_id)
                .await
                .map_err(RpcError::Anyhow)?,
        )
    }
    ...
    async fn get_checkpoint_tree_merkle_proof(&self, checkpoint_id: u64, leaf_checkpoint_id: u64) -> RpcResult<MerkleProofCore<QHashOut<F>>> {
        Ok(
            PsyRealmStoreReaderAsync::get_checkpoint_tree_merkle_proof(&self.store, checkpoint_id, leaf_checkpoint_id)
                .await
                .map_err(RpcError::Anyhow)?,
        )
    }
    ...
    async fn get_checkpoint_global_state_roots(&self, checkpoint_id: u64) -> RpcResult<PsyCheckpointGlobalStateRoots<F>> {
        Ok(PsyRealmStoreReaderAsync::get_checkpoint_global_state_roots(&self.store, checkpoint_id)
            .await
            .map_err(RpcError::Anyhow)?)
    }

    async fn get_user_leaf_data(&self, checkpoint_id: u64, user_id: u64) -> RpcResult<PsyUserLeaf<F>> {
        Ok(PsyRealmStoreReaderAsync::get_user_leaf_data(&self.store, checkpoint_id, user_id)
            .await
            .map_err(RpcError::Anyhow)?)
    }
    ...
    async fn get_user_contract_state_tree_root(&self, checkpoint_id: u64, user_id: u64, contract_id: u32) -> RpcResult<QHashOut<F>> {
        Ok(
            PsyRealmStoreReaderAsync::get_user_contract_state_tree_root(&self.store, checkpoint_id, user_id, contract_id)
                .await
                .map_err(RpcError::Anyhow)?,
        )
    }
    ...
    async fn get_user_contract_state_tree_leaf_hash(
        ...
        Ok(
            PsyRealmStoreReaderAsync::get_user_contract_state_tree_leaf_hash(&self.store, checkpoint_id, user_id, contract_id, height, leaf_id)
                .await
                .map_err(RpcError::Anyhow)?,
        )
    }
    ...
    async fn get_user_contract_state_tree_merkle_proof(
        ...
        Ok(
            PsyRealmStoreReaderAsync::get_user_contract_state_tree_merkle_proof(&self.store, checkpoint_id, user_id, contract_id, height, leaf_id)
                .await
                .map_err(RpcError::Anyhow)?,
        )
    }
    ...
    async fn get_user_contract_tree_root(&self, checkpoint_id: u64, user_id: u64) -> RpcResult<QHashOut<F>> {
        Ok(PsyRealmStoreReaderAsync::get_user_contract_tree_root(&self.store, checkpoint_id, user_id)
            .await
            .map_err(RpcError::Anyhow)?)
    }
    ...
    async fn get_user_contract_tree_leaf_hash(&self, checkpoint_id: u64, user_id: u64, contract_id: u32) -> RpcResult<QHashOut<F>> {
        Ok(
            PsyRealmStoreReaderAsync::get_user_contract_tree_leaf_hash(&self.store, checkpoint_id, user_id, contract_id)
                .await
                .map_err(RpcError::Anyhow)?,
        )
    }
    ...
    async fn get_user_contract_tree_merkle_proof(
        ...
        Ok(
            PsyRealmStoreReaderAsync::get_user_contract_tree_merkle_proof(&self.store, checkpoint_id, user_id, contract_id)
                .await
                .map_err(RpcError::Anyhow)?,
        )
    }
    ...
    async fn get_user_tree_root(&self, checkpoint_id: u64) -> RpcResult<QHashOut<F>> {
        Ok(PsyRealmStoreReaderAsync::get_user_tree_root(&self.store, checkpoint_id)
            .await
            .map_err(RpcError::Anyhow)?)
    }

    async fn get_user_tree_leaf_hash(&self, checkpoint_id: u64, user_id: u64) -> RpcResult<QHashOut<F>> {
        Ok(PsyRealmStoreReaderAsync::get_user_tree_leaf_hash(&self.store, checkpoint_id, user_id)
            .await
            .map_err(RpcError::Anyhow)?)
    }
    ...
    async fn get_user_bottom_tree_merkle_proof(&self, root_level: u8, checkpoint_id: u64, user_id: u64) -> RpcResult<MerkleProofCore<QHashOut<F>>> {
        Ok(
            PsyRealmStoreReaderAsync::get_user_bottom_tree_merkle_proof(&self.store, root_level, checkpoint_id, user_id)
                .await
                .map_err(RpcError::Anyhow)?,
        )
    }
    ...
    async fn get_user_sub_tree_merkle_proof(
        ...
        Ok(
            PsyRealmStoreReaderAsync::get_user_sub_tree_merkle_proof(&self.store, checkpoint_id, root_level, leaf_level, leaf_index)
                .await
                .map_err(RpcError::Anyhow)?,
        )
    }
    ...
    async fn get_user_tree_merkle_proof(&self, checkpoint_id: u64, user_id: u64) -> RpcResult<MerkleProofCore<QHashOut<F>>> {
        Ok(PsyRealmStoreReaderAsync::get_user_tree_merkle_proof(&self.store, checkpoint_id, user_id)
            .await
            .map_err(RpcError::Anyhow)?)
    }
}
