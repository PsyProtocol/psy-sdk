...
impl RabbitStreamQueue {
    ...
    pub async fn push_tasks(&mut self, tasks: &[QProvingJobDataID]) -> anyhow::Result<()> {
        ...
        let producer: Producer<NoDedup> = self.environment.producer().build(&self.stream).await?;
    ...
    pub async fn get_next_task(&mut self) -> anyhow::Result<QProvingJobDataID> {
        ...
        let mut consumer = self.environment
            .consumer()
            .offset(OffsetSpecification::First)
            .build(&self.stream)
            .await?;
        ...
        while let Some(Ok(delivery)) = consumer.next().await {
            ...
            .data() {
                Some(d) => {
                    println!("got msgg");
                    Some(QProvingJobDataID::try_from_byte_vec(d)?)},
                None => None,
            };
            if data.is_some() {
        ...
        return Ok(QProvingJobDataID::notify_block_complete(0));
    }
