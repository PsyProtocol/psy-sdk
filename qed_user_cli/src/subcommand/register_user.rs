...
use qed_prover::wallet::simple_sign::{
    SimpleSoftwareDefinedCircuit, SoftwareDefinedSignGadget, SoftwareDefinedSignTrait,
};
use qed_prover::wallet::software_defined_circuit::SoftwareDefinedCircuit;
...
pub fn get_sdc_public_key(private_key: &QHashOut<GoldilocksField>) -> QHashOut<GoldilocksField> {
    SoftwareDefinedSignGadget::get_public_key::<GoldilocksField, PoseidonHash>(
...
pub fn run(args: RegisterUserArgs) -> Result<()> {
    ...
    const D: usize = 2;
    let sdc_fingerprint =
        SimpleSoftwareDefinedCircuit::<C, D, SoftwareDefinedSignGadget>::new().get_fingerprint();

    let user_sdc: DPNFunctionCircuitDefinition =
        serde_json::from_str(&std::fs::read_to_string("sdc.json")?)?;

    let sdc = SoftwareDefinedCircuit::<C, D>::new(
        &user_sdc,
    ...
    let public_key_param = match KeyType::from(args.key_type.clone()) {
        ...
        KeyType::SoftwareDefined => SoftwareDefinedSignGadget::get_public_key::<
            GoldilocksField,
            PoseidonHash,
        >(private_key_base.into())
        .into(),
        KeyType::SoftwareDefinedV2 => get_sdc_public_key(&private_key_base),
    };

    let fingerprint = match KeyType::from(args.key_type.clone()) {
        KeyType::ZK => {
        ...
        KeyType::SoftwareDefined => Ok(sdc_fingerprint),
        KeyType::SoftwareDefinedV2 => Ok(sdc.get_fingerprint()),
    }?;
