...
pub fn run(args: LPSArgs) -> anyhow::Result<()> {
    let contract_call_args: Vec<ContractCallArgs> =
        serde_json::from_str(&std::fs::read_to_string(&args.contract_call_path)?)?;

    let mut st_provider = StorageProvider::new(&args.store_config_path)?;
    ...
    let st_provider2 = StorageProvider::new(&args.store_config_path)?;
    ...
    let rpc_provider = RpcProvider::new(&args.rpc_config_path)?;

    rpc_provider.submit_end_cap_proof::<F>(QSubmitEndCapRPCRequest {
        user_ec_input,
        proof: end_cap_proof,
    })?;
