...
#[derive(Debug)]
pub struct PsyProofMinifierChain<const D: usize, F: RichField + Extendable<D>, C: GenericConfig<D, F = F>>
where
    <C as GenericConfig<D>>::Hasher: AlgebraicHasher<F>,
{
    pub minifiers: Vec<PsyProofMinifier<D, F, C>>,
}

impl<const D: usize, F: RichField + Extendable<D>, C: GenericConfig<D, F = F>> PsyProofMinifierChain<D, F, C>
where
    <C as GenericConfig<D>>::Hasher: AlgebraicHasher<F>,
{
    pub fn new_with_cfg(
        ...
        let mut minifiers = vec![PsyProofMinifier::<D, F, C>::new_with_cfg(
        ...
        for i in 1..minifier_configs.len() {
            minifiers.push(PsyProofMinifier::<D, F, C>::new_with_cfg(
    ...
    pub fn new_with_cfg_customizer<PMCC: PMCircuitCustomizer<F, D>>(
        ...
        let mut minifiers = vec![if n_minifiers == 1 {
            PsyProofMinifier::<D, F, C>::new_with_cfg_customizer(
            ...
            PsyProofMinifier::<D, F, C>::new_with_cfg(
                CircuitConfig::standard_recursion_config(),
                base_circuit_verifier_data,
                base_circuit_common_data,
                None,
        ...
        for i in 1..n_minifiers {
            if i == (n_minifiers - 1) {
                minifiers.push(PsyProofMinifier::<D, F, C>::new_with_cfg_customizer(
                    CircuitConfig::standard_recursion_config(),
                    &minifiers[i - 1].circuit_data.verifier_only,
                    &minifiers[i - 1].circuit_data.common,
                    None,
    ...
    pub fn new(
        ...
        let mut minifiers = vec![PsyProofMinifier::<D, F, C>::new(base_circuit_verifier_data, base_circuit_common_data)];
        for i in 1..n_minifiers {
            minifiers.push(PsyProofMinifier::<D, F, C>::new(
                &minifiers[i - 1].circuit_data.verifier_only,
                &minifiers[i - 1].circuit_data.common,
            ));
        }
