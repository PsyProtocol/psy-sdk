...
pub struct Sha256Hasher192;
impl GenericCircuitMerkleHasher<Hash192Target> for Sha256Hasher192 {
    fn gc_two_to_one<F: QRichField + Extendable<D>, const D: usize>(
        builder: &mut CircuitBuilder<F, D>,
        left: Hash192Target,
        right: Hash192Target,
    ) -> Hash192Target {
        builder.two_to_one_truncated_sha256(left, right)
    }
...
mod tests {
    ...
    fn test_truncated_sha256_two_to_one() {
        ...
        for t in tests {
            let left = Hash192::from_hex_string(t[0]).unwrap();
            let right = Hash192::from_hex_string(t[1]).unwrap();
            let expected_output = Hash192::from_hex_string(t[2]).unwrap();
