...
pub trait CircuitBuilderConnectHelpers<F: RichField + Extendable<D>, const D: usize> {
    ...
    fn connect_templated_array(&mut self, template_start: &[u64], template_middle: &[Target], template_end: &[u64], target_array: &[Target]);
    fn connect_constant(&mut self, x: Target, value: u64);
}

impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilderConnectHelpers<F, D> for CircuitBuilder<F, D> {
    fn connect_if_true(&mut self, condition: BoolTarget, x: Target, y: Target) {
    ...
    fn connect_hashes_switch_on_enum(&mut self, switch: BoolTarget, value: HashOutTarget, on_hashes: &[HashOutTarget], off_hash: HashOutTarget) {
        ...
        let actual_hash_target = HashOutTarget {
            elements: core::array::from_fn(|i| self.select(switch, on_hash.elements[i], off_hash.elements[i])),
        };
    ...
    fn connect_zero_if_true(&mut self, condition: BoolTarget, x: Target) {
        let zero = self.zero();
        let x_alt = self.mul(condition.target, x);
        self.connect(zero, x_alt);
    }
}
