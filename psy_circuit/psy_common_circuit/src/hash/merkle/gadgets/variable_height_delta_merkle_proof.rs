...
#[derive(Debug, Clone)]
pub struct VariableHeightDeltaMerkleProofGadget {
...
impl VariableHeightDeltaMerkleProofGadget {
    ...
    pub fn compute_root_bits<H: AlgebraicHasher<F>, F: RichField + Extendable<D>, const D: usize>(
        ...
        for (ind, (&bit, &sibling)) in index_bits.iter().zip(siblings).enumerate() {
            ...
            let old_left = builder.select_hash(bit, sibling, old_state);
            let old_right = builder.select_hash(bit, old_state, sibling);
            let old_proposed_state = builder.hash_two_to_one::<H>(old_left, old_right);

            old_state = builder.select_hash(is_remaining_levels_zero, old_state, old_proposed_state);

            let new_left = builder.select_hash(bit, sibling, new_state);
            let new_right = builder.select_hash(bit, new_state, sibling);
            let new_proposed_state = builder.hash_two_to_one::<H>(new_left, new_right);
...
mod tests {
    ...
    use super::VariableHeightDeltaMerkleProofGadget;
    ...
    struct TestVariableHeightDeltaMerkleProofCircuit {
        pub variable_delta_merkle_proof_gadget: VariableHeightDeltaMerkleProofGadget,
        pub height: Target,
    ...
    impl TestVariableHeightDeltaMerkleProofCircuit {
        pub fn new(max_height: usize) -> Self {
            ...
            let variable_delta_merkle_proof_gadget =
                VariableHeightDeltaMerkleProofGadget::add_virtual_to_full::<PoseidonHash, F, D>(&mut builder, max_height, Some(height));
    ...
    struct TestVariableHeightSubRootMerkleProofCircuit {
        pub variable_dmp_gadget_a: VariableHeightDeltaMerkleProofGadget,
        pub variable_dmp_gadget_b: VariableHeightDeltaMerkleProofGadget,
        pub level_a: Target,
        pub level_b: Target,
    ...
    impl TestVariableHeightSubRootMerkleProofCircuit {
        pub fn new(max_height: usize) -> Self {
            ...
            let variable_dmp_gadget_a = VariableHeightDeltaMerkleProofGadget::add_virtual_to_full_with_subtree_root_index::<PoseidonHash, F, D>(
                &mut builder,
                max_height,
                Some(height_a),
            );

            let variable_dmp_gadget_b = VariableHeightDeltaMerkleProofGadget::add_virtual_to_full_with_subtree_root_index::<PoseidonHash, F, D>(
                &mut builder,
                max_height,
                Some(height_b),
            );
