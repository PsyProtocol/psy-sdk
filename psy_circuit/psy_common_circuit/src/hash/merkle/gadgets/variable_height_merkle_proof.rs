...
impl VariableHeightMerkleProofGadget {
    ...
    pub fn add_virtual_to_full_with_subtree_root_index<H: AlgebraicHasher<F>, F: RichField + Extendable<D>, const D: usize>(
        ...
        let (root, sub_tree_root_index, sub_tree_root_path_direction, sub_tree_root_direct_child_in_path_value) =
            Self::compute_root_and_subtree_root_index::<H, F, D>(builder, index, value, &siblings, height);
    ...
    pub fn compute_root_bits<H: AlgebraicHasher<F>, F: RichField + Extendable<D>, const D: usize>(
        ...
        for (&bit, &sibling) in index_bits.iter().zip(siblings) {
            ...
            let proposed_state = builder.two_to_one_swapped::<H>(state, sibling, bit);
            ...
            is_remaining_levels_zero = builder.or(is_remaining_levels_zero, is_cur_remaining_levels_zero);
        }
        state
    }
    pub fn compute_root_bits_and_subtree_root_index_new<H: AlgebraicHasher<F>, F: RichField + Extendable<D>, const D: usize>(
        builder: &mut CircuitBuilder<F, D>,
        index_bits: &[BoolTarget],
        value: HashOutTarget,
        siblings: &[HashOutTarget],
        ...
        for (&bit, &sibling) in index_bits.iter().zip(siblings) {
            ...
            let left = builder.select_hash(bit, sibling, state);
            let right = builder.select_hash(bit, state, sibling);
            let proposed_state = builder.hash_two_to_one::<H>(left, right);
...
mod tests {
    ...
    use crate::{
        builder::comparison::CircuitBuilderComparison, hash::merkle::gadgets::variable_height_merkle_proof::VariableHeightMerkleProofGadget,
    };
    ...
    impl TestVariableHeightMerkleProofCircuit {
        pub fn new(max_height: usize) -> Self {
            ...
            let variable_merkle_proof_gadget =
                VariableHeightMerkleProofGadget::add_virtual_to_full::<PoseidonHash, F, D>(&mut builder, max_height, Some(height));
        ...
        pub fn prove(&self, height: usize, merkle_proof: &MerkleProofCore<QHashOut<F>>) -> anyhow::Result<ProofWithPublicInputs<F, C, D>> {
            ...
            self.variable_merkle_proof_gadget
                .set_witness_core_proof_q_generic(&mut pw, merkle_proof)?;
    ...
    impl TestVariableHeightSubRootMerkleProofCircuit {
        pub fn new(max_height: usize) -> Self {
            ...
            builder.connect_hashes(variable_merkle_proof_gadget_a.root, variable_merkle_proof_gadget_b.root);

            builder.register_public_inputs(
                &vec![
                    vec![sub_root_level, variable_merkle_proof_gadget_a.sub_tree_root_index.unwrap()],
        ...
        pub fn prove(
            ...
            self.variable_merkle_proof_gadget_a
                .set_witness_core_proof_q_generic(&mut pw, merkle_proof_a)?;
            self.variable_merkle_proof_gadget_b
                .set_witness_core_proof_q_generic(&mut pw, merkle_proof_b)?;
            self.circuit_data.prove(pw)
        }
    }
    ...
    pub fn generate_random_sub_tree_merkle_proofs_at_height(
        height: usize,
    ...
    fn test_variable_merkle_proof_sub_tree_circuit() {
        ...
        let merkle_proofs = (1..32)
            .map(|level| generate_random_sub_tree_merkle_proofs_at_height(level, level * 100))
            .collect::<Vec<_>>();
    ...
    fn test_variable_merkle_proof_circuit() {
        ...
        let merkle_proofs = (1..32)
            .map(|level| generate_random_merkle_proofs_at_height(level, 2))
            .flatten()
            .collect::<Vec<_>>();
