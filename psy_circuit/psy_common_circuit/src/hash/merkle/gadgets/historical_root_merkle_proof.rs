...
impl HistoricalRootMerkleProofGadget {
    ...
    pub fn add_virtual_to_zero_gte<H: MerkleZeroHasher<HashOut<F>> + AlgebraicHasher<F>, F: RichField + Extendable<D>, const D: usize>(
        ...
        let (historical_root, current_root) =
            Self::compute_root_historical_root_with_historical_state::<H, F, D>(builder, index, current_value, &siblings, zero_hash);
        tracing::debug!(
            "🔍 Historical Merkle Proof GTE - computed roots: current={:?}, historical={:?}, index={:?}, height={}",
            current_root,
            historical_root,
            index,
            height
    ...
    // calculate the normal merkle root + a historical merkle root where all the
    // leaves with index > `gadget.index` set to zero
    pub fn add_virtual_to_zero_gt<H: MerkleZeroHasher<HashOut<F>> + AlgebraicHasher<F>, F: RichField + Extendable<D>, const D: usize>(
        builder: &mut CircuitBuilder<F, D>,
        height: usize,
    ) -> Self {
        let index = builder.add_virtual_target();
        ...
        let (historical_root, current_root) =
            Self::compute_root_historical_root_with_historical_state::<H, F, D>(builder, index, current_value, &siblings, current_value);
        tracing::debug!(
            "🔍 Historical Merkle Proof GT - computed roots: current={:?}, historical={:?}, index={:?}, height={}",
            current_root,
            historical_root,
            index,
            height
    ...
    >(
        ...
        Self::compute_root_historical_root_bits_with_historical_state::<H, F, D>(builder, &index_bits, value, siblings, start_historical_state)
    }
    ...
    >(
        ...
        for (&bit, &sibling) in index_bits.iter().zip(siblings) {
            ...
            // if the right node is not on the path, then it should historically be a zero
            // hash
    ...
    pub fn set_witness_proof_core<W: Witness<F>, F: RichField>(&self, witness: &mut W, proof: &MerkleProofCore<QHashOut<F>>) -> anyhow::Result<()> {
        tracing::debug!(
            "🔍 Historical Merkle Proof set_witness - index: {}, value: {}, siblings_len: {}",
            proof.index,
            proof.value,
            proof.siblings.len()
        );

        self.set_witness_generic::<W, F>(witness, F::from_noncanonical_u64(proof.index), proof.value, &proof.siblings)
    }
}
...
mod tests {
    ...
    use psy_crypto::hash::merkle::{
        core::{compute_historical_and_current_merkle_roots_core, MerkleProofCore},
    ...
    fn create_historical_mp_circuit_for_proof_a(mp: &MerkleProofCore<QHashOut<F>>) -> QHashOut<F> {
        assert!(
            mp.verify::<PoseidonHash>(),
            "invalid proof passed to create_historical_mp_circuit_for_proof_a"
        );

        let (expected_historical_root, expected_current_root) = compute_historical_and_current_merkle_roots_core::<QHashOut<F>, PoseidonHash>(mp);

        // this should never happpen if mp.root and
        // compute_historical_and_current_merkle_roots_core is correct
        assert_eq!(
            expected_current_root, mp.root,
            "compute_historical_and_current_merkle_roots_core computed a root which doesn't match merkle_proof.root"
        );
        ...
        let historical_gadget = HistoricalRootMerkleProofGadget::add_virtual_to_zero_gte::<PoseidonHash, F, D>(&mut builder, tree_height);
        ...
        assert_eq!(
            ...
            expected_historical_root.0.elements,
            "expected_historical_root does not match proof output"
        );
        assert_eq!(
            proof.public_inputs[4..8],
            expected_current_root.0.elements,
            "expected_current_root does not match proof output"
        );
    ...
    fn test_historical_merkle_proof_gadget_simple() {
        ...
        for i in 0..128 {
            historical_roots.push(merkle_tree.set_leaf(i, QHashOut::from_values(100 + i, 5, 16, i)).old_root);
        }

        for (i, historical_root) in historical_roots.into_iter().enumerate() {
            let merkle_proof = merkle_tree.get_leaf(i as u64);
            let computed_historical_root = create_historical_mp_circuit_for_proof_a(&merkle_proof);
