...
impl ByteTargetVectorBuilder {
    ...
    pub fn write_u64_bytes_be<F: RichField + Extendable<D>, const D: usize>(&mut self, builder: &mut CircuitBuilder<F, D>, x: Target) -> &mut Self {
        let mut tmp = builder.split_le_base::<8>(x, 8);
        tmp.reverse();
        self.write_slice(&tmp);
        self
    }
    pub fn write_u64_bytes_le<F: RichField + Extendable<D>, const D: usize>(&mut self, builder: &mut CircuitBuilder<F, D>, x: Target) -> &mut Self {
    ...
    pub fn write_constant_u8<F: RichField + Extendable<D>, const D: usize>(&mut self, builder: &mut CircuitBuilder<F, D>, x: u8) -> &mut Self {
        self.data.push(builder.constant(F::from_canonical_u8(x)));
        self
    }

    pub fn write_constant_u32_le<F: RichField + Extendable<D>, const D: usize>(&mut self, builder: &mut CircuitBuilder<F, D>, x: u32) -> &mut Self {
        self.data.extend_from_slice(&builder.constant_u32_bytes_le(x));
        self
    }

    pub fn write_constant_u32_be<F: RichField + Extendable<D>, const D: usize>(&mut self, builder: &mut CircuitBuilder<F, D>, x: u32) -> &mut Self {
        self.data.extend_from_slice(&builder.constant_u32_bytes_be(x));
        self
    }
    pub fn write_constant_u64_le<F: RichField + Extendable<D>, const D: usize>(&mut self, builder: &mut CircuitBuilder<F, D>, x: u64) -> &mut Self {
        self.data.extend_from_slice(&builder.constant_u64_bytes_le(x));
        self
    }

    pub fn write_constant_u64_be<F: RichField + Extendable<D>, const D: usize>(&mut self, builder: &mut CircuitBuilder<F, D>, x: u64) -> &mut Self {
        self.data.extend_from_slice(&builder.constant_u64_bytes_be(x));
        self
    }

    pub fn write_constant_varuint<F: RichField + Extendable<D>, const D: usize>(&mut self, builder: &mut CircuitBuilder<F, D>, x: u64) -> &mut Self {
        self.data.extend_from_slice(&builder.constant_bytes(&encode_varuint(x)));
        self
    }
    pub fn write_var_slice<F: RichField + Extendable<D>, const D: usize>(
        ...
        self.write_constant_varuint(builder, byte_targets.len() as u64).write_slice(byte_targets)
    }
