    ...
    C::Hasher: AlgebraicHasher<C::F>,
{
    pub fn new(merkle_tree_height: usize) -> Self {
        ...
        let mp_gadget = MerkleProofGadget::add_virtual_to::<C::Hasher, C::F, D>(&mut builder, merkle_tree_height);
        let combined_state_hash = builder.hash_two_to_one::<C::Hasher>(mp_gadget.root, mp_gadget.value);
    ...
    pub fn prove_base(&self, merkle_proof: &MerkleProofCore<QHashOut<C::F>>) -> ProofWithPublicInputs<C::F, C, D> {
        let mut witness = PartialWitness::new();
        self.mp_gadget.set_witness_core_proof_q(&mut witness, merkle_proof).unwrap();
...
async fn run_prove_agg_example_2() -> anyhow::Result<()> {
    ...
    let mut recursion_mgr = PortableQTreeRecursionManager::new(proof_tree_height).await;
    ...
    println!("recursion_mgr.leaf_proofs.len() = {}", recursion_mgr.leaf_proofs.len());
    println!("recursion_mgr.agg_proofs.len() = {}", recursion_mgr.agg_proofs.len());
...
async fn run_prove_agg_example_1() -> anyhow::Result<()> {
    ...
    let circuits = PortableQTreeRecursionCircuits::new(
        proof_tree_height,
        1337,
        ex_leaf_circuit.base_circuit_data.verifier_only.constants_sigmas_cap.height(),
