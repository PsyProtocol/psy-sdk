...
use psy_common_circuit::{
    hash::merkle::gadgets::sub_tree_update_proof_opt::UpdateNearestCommonAncestorProofOptGadget,
...
impl TwoNCAStateTransitionGadget {
    pub fn add_virtual_to<H: AlgebraicHasher<F>, F: RichField + Extendable<D>, const D: usize>(
        ...
        let update_nca_proof_gadget =
            UpdateNearestCommonAncestorProofOptGadget::add_virtual_to_full::<H, F, D>(builder, GLOBAL_USER_TREE_HEIGHT as usize);

        // TODO: WHY WAS THIS REMOVED, I am adding back!?!!, we need to make sure a and
        // b have the same checkpoint root
        builder.connect_hashes(a_header.checkpoint_tree_root, b_header.checkpoint_tree_root);

        builder.connect_hashes(a_header.guta_circuit_whitelist, b_header.guta_circuit_whitelist);

        builder.connect_hashes(a_header.state_transition.old_node_value, update_nca_proof_gadget.child_a.old_value);
        builder.connect_hashes(a_header.state_transition.new_node_value, update_nca_proof_gadget.child_a.new_value);
        builder.connect(a_header.state_transition.node_index, update_nca_proof_gadget.child_a.index);

        builder.connect(a_header.state_transition.node_level, update_nca_proof_gadget.level_a);

        builder.connect_hashes(b_header.state_transition.old_node_value, update_nca_proof_gadget.child_b.old_value);
        builder.connect_hashes(b_header.state_transition.new_node_value, update_nca_proof_gadget.child_b.new_value);
        builder.connect(b_header.state_transition.node_index, update_nca_proof_gadget.child_b.index);

        builder.connect(b_header.state_transition.node_level, update_nca_proof_gadget.level_b);
    ...
    pub fn set_witness_params<W: Witness<F>, F: RichField>(
        ...
        self.update_nca_proof_gadget
            .set_witness_params(witness, child_a, child_b, nearest_common_ancestor_level)
    }
    pub fn set_witness_partial<W: Witness<F>, F: RichField>(
        ...
        self.update_nca_proof_gadget.set_witness_partial(witness, input)
    }
    pub fn set_witness_full<W: Witness<F>, F: RichField>(
        ...
        self.update_nca_proof_gadget.set_witness_full(witness, input)
    }
}
