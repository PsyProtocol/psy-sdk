...
use psy_common_circuit::{builder::hash::core::CircuitBuilderHashCore, treeprover::subtree::gadgets::subtree_core::SubTreeNodeStateTransitionGadget};
...
impl GlobalUserTreeAggregatorHeaderGadget {
    ...
    pub fn set_witness<F: RichField>(&self, witness: &mut impl Witness<F>, target: &GlobalUserTreeAggregatorHeader<F>) -> anyhow::Result<()> {
        witness.set_hash_target(self.guta_circuit_whitelist, target.guta_circuit_whitelist.0)?;
    ...
    pub fn to_hash<H: AlgebraicHasher<F>, F: RichField + Extendable<D>, const D: usize>(&self, builder: &mut CircuitBuilder<F, D>) -> HashOutTarget {
        ...
        let state_transition_and_stats_hash = builder.hash_two_to_one::<H>(state_transition_hash, stats_hash);
        let state_stats_checkpoint_hash = builder.hash_two_to_one::<H>(self.checkpoint_tree_root, state_transition_and_stats_hash);

        builder.hash_two_to_one::<H>(self.guta_circuit_whitelist, state_stats_checkpoint_hash)
    }
}
