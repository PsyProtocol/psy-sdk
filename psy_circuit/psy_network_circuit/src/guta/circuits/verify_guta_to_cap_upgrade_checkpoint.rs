...
use psy_data::guta::{header::GlobalUserTreeAggregatorHeader, proof_input::VerifyGUTAToCapUpgradeCheckpointCircuitInputSimple};

use crate::{
    gadgets::qdata::pm_jobs_completed_stats::PMJobsCompletedStatsGadget, guta::gadgets::verify_guta_proof_to_line::VerifyGUTAProofToLineGadget,
};
    ...
    C::Hasher: AlgebraicHasher<C::F> + MerkleZeroHasher<HashOut<C::F>>,
{
    pub fn new(guta_proof_common_data: &CommonCircuitData<C::F, D>, guta_proof_verifier_data_cap_height: usize) -> Self {
        ...
        let upgrade_checkpoint_historical_merkle_proof_gadget =
            HistoricalRootMerkleProofGadget::add_virtual_to_zero_gt::<C::Hasher, C::F, D>(&mut builder, CHECKPOINT_TREE_HEIGHT as usize);
        ...
        // verify_to_line_gadget.get_guta_header_line().to_hash::<C::Hasher, C::F,
        // D>(&mut builder);
...
impl<S: QProofStoreReaderAsync + Send + Sync, L: CircuitInfoLibrary<C, D> + Send + Sync, C: GenericConfig<D> + 'static, const D: usize>
    QStandardCircuitProvableWithProofStoreAndRefLibraryAsync<S, L, C, D> for GUTAVerifyGUTAToCapUpgradeCheckpointCircuit<C, D>
where
    C::Hasher: AlgebraicHasher<C::F> + MerkleZeroHasher<HashOut<C::F>>,
{
