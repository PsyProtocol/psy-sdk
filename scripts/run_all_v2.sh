...
REALM_PROCESSOR_V2_LOG="$LOG_DIR/realm-processor-v2.log"
COORDINATOR_EDGE_LOG="$LOG_DIR/coordinator-edge.log"
REALM_EDGE_V2_LOG="$LOG_DIR/realm-edge-v2.log"

REALM_PROCESSOR1_V2_LOG="$LOG_DIR/realm-processor1-v2.log"
REALM_EDGE1_V2_LOG="$LOG_DIR/realm-edge1-v2.log"
...
: > "$REALM_PROCESSOR_V2_LOG"
: > "$COORDINATOR_EDGE_LOG"
: > "$REALM_EDGE_V2_LOG"
: > "$REALM_PROCESSOR1_V2_LOG"
: > "$REALM_EDGE1_V2_LOG"
...
run_service "make run-realm-processor-v2" "realm-processor-v2" "$REALM_PROCESSOR_V2_LOG" &
PIDS+=($!)
run_service "make run-realm-processor1-v2" "realm-processor1-v2" "$REALM_PROCESSOR1_V2_LOG" &
PIDS+=($!)

# Group 2: Start edge services (depend on processors)
sleep 8
run_service "make run-realm-edge-v2" "realm-edge-v2" "$REALM_EDGE_V2_LOG" &
PIDS+=($!)
run_service "make run-realm-edge1-v2" "realm-edge1-v2" "$REALM_EDGE1_V2_LOG" &
PIDS+=($!)
