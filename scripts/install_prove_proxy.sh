...
Examples:
    ...
    sudo $0 /path/to/psy_user_cli --create-user

    # Custom installation
    sudo $0 ./psy_user_cli --user myuser --num-proxies 5 --base-port 8080

    # Upgrade existing installation
    sudo $0 /new/version/psy_user_cli --upgrade
...
echo "  $BINARY_PATH → $BIN_DIR/psy_user_cli"
copy_with_backup "$BINARY_PATH" "$BIN_DIR/psy_user_cli"
run_cmd chmod 755 "$BIN_DIR/psy_user_cli"
...
BINARY_PATH="$BIN_DIR/psy_user_cli"
RUN_DIR="$RUN_DIR"
LOG_DIR="$LOG_DIR"
CONFIG_DIR="$ETC_DIR"
VAR_DIR="$VAR_DIR"
...
BASE_PORT=$BASE_PORT
#LOG_LEVEL="psy_ups_circuit=trace,psy_dpn_circuit=trace,psy_user_cli=debug,psy_prover=trace"
