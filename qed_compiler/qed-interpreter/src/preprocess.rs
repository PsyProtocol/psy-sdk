...
impl<'a> StorageProcessor<'a> {
    ...
    >(
        ...
        if let UncheckedType::Array(elem_ty, size, _loc) = field_type {
            ...
            Some(TraitImplNode {
                ...
                ty: UncheckedType::Array(elem_ty.clone(), *size, attr.location),
                body: methods,
                comments: vec![],
                location: attr.location,
                is_generated: false,
    ...
    >(
        ...
        for (field_name, field) in &struct_node.fields {
            let transformed_type =
                if new_name_suffix.is_some() || attr.name == ctx.intern("contract") {
                    // For Ref-suffixed structs or #[contract], wrap field type in StorageRef<T>
        ...
        StructNode {
            ...
            attrs: if new_name_suffix.is_some() {
                ...
                struct_node
                    ...
                    .iter()
                    .filter(|a| {
                        let attr_id = a.name.id;
                        attr_id != ctx.intern("StorageRef") && attr_id != ctx.intern("contract")
                    })
                    .cloned()
                    .collect()
            },
    ...
    >(
        ...
        for (field_name, field) in &struct_node.fields {
            ...
            let storage_ref_path = PathNode {
                root: None,
                segments: vec![],
                target: UncheckedType::Generic(
                    Identifier::new(ctx.intern("StorageRef"), attr.location),
                    vec![inner_ty.clone()],
                    attr.location,
                ),
            ...
            let new_path = PathNode {
                root: Some(UncheckedType::Path(Box::new(storage_ref_path.clone()))),
                segments: vec![],
    ...
    >(
        ...
        let i_ident = ctx.intern("I");
        let variable = ctx.alloc_expression(ExprNode::Path(PathNode {
            root: Some(UncheckedType::Path(Box::new(PathNode::from_target(
                UncheckedType::Basic(Identifier::new(i_ident, attr.location)),
            )))),
            segments: vec![],
        ...
        let f = FunctionNode {
            ...
            parameters: vec![
                FunctionParameter::new(
                    offset_ident,
                    TypeQualifier::new(false, attr.location),
                    UncheckedType::Path(Box::new(PathNode::from_target(UncheckedType::Basic(
            ...
            generic_parameters: vec![],
            body: Some(block),
            return_type: Some(UncheckedType::Path(Box::new(PathNode::from_target(
                UncheckedType::Basic(Identifier::new(ctx.intern("I"), attr.location)),
            )))),
            qualifier: Qualifier {
                is_extern: false,
                is_const: false,
        ...
        let f = FunctionNode {
            ...
            parameters: vec![
                ...
                FunctionParameter::new(
                    ...
                    TypeQualifier::new(false, attr.location),
                    UncheckedType::Path(Box::new(PathNode::from_target(UncheckedType::Basic(
                        Identifier::new(ctx.intern("I"), attr.location),
                    )))),
                    attr.location,
                ),
            ],
    ...
    >(
        ...
        let read_ident = Identifier::new(ctx.intern("read_at"), attr.location);
        let variable = ctx.alloc_expression(ExprNode::Path(PathNode {
            root: Some(field_type.clone()),
            segments: vec![],
            target: UncheckedType::Basic(read_ident),
        ...
        let elem_ty = if let UncheckedType::Array(elem_ty, _, _) = field_type {
            ...
            panic!("generate_getter_at called on non-array type");
        };

        let read_call = CallNode {
            callee: variable,
            generic_parameters: vec![],
            args: vec![offset, index_expr],
            location: attr.location,
        };
        let read_expr = ctx.alloc_expression(ExprNode::Call(read_call));
    ...
    >(
        ...
        let write_ident = Identifier::new(ctx.intern("write_at"), attr.location);
        let variable = ctx.alloc_expression(ExprNode::Path(PathNode {
            root: Some(field_type.clone()),
            segments: vec![],
            target: UncheckedType::Basic(write_ident),
        ...
        let elem_ty = if let UncheckedType::Array(elem_ty, _, _) = field_type {
            ...
            panic!("generate_setter_at called on non-array type");
        };
    ...
    fn visit_struct(
        ...
        for attr in &s.attrs {
                    ...
                    .any(|p| p == &storage_ref_attribute_id)
            {
                ...
                // Insert the new struct at the end of the module
            ...
            // Handle #[contract]
            if attr.name == contract_attribute_id {
                // Transform the original struct to use StorageRef<T> for fields
                let transformed_struct = self.transform_struct_to_storage_ref(&s, attr, ctx, None);
                ctx.replace_definition(node, DefinitionNode::Struct(transformed_struct));

                // Generate impl with new method
                let mut methods = Vec::new();
                methods.push(self.generate_new_method(&s, attr, ctx));
                let impl_node = ImplNode {
                    associated_types: IndexMap::new(),
                    generic_parameters: s.generic_parameters.clone(),
                    ty: UncheckedType::Basic(s.name),
                    body: methods,
                    comments: vec![],
                    location: attr.location,
                    is_generated: false,
                ...
                // Generate Storage trait implementation with Value = xxx
                let storage_impl = self.generate_storage_impl(
                    &s,
                    attr,
                    ctx,
