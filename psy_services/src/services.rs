...
impl CheckpointRewardService {
    ...
    pub async fn report_checkpoint_stats(&self, stats: CheckpointLeafStat) -> anyhow::Result<CheckpointStats> {
        info!(
            "Reporting checkpoint stats for checkpoint {}: fees={}, transactions={}",
            stats.checkpoint_id, stats.fees_collected, stats.total_transactions
        );

        let checkpoint_stats = CheckpointStatsRepository::create(&self.pool, &stats).await?;

        info!("Successfully reported checkpoint stats for checkpoint {}", checkpoint_stats.checkpoint_id);

        Ok(checkpoint_stats)
    }

    /// Report worker job events (called by watcher after 3+ block
    /// confirmations)
    pub async fn report_worker_job_events(&self, events: Vec<CreateWorkerJobEvent>) -> anyhow::Result<Vec<WorkerJobEvent>> {
        if events.is_empty() {
            return Ok(vec![]);
        }

        let checkpoint_id = events[0].checkpoint_id;
        info!("Reporting {} worker job events for checkpoint {}", events.len(), checkpoint_id);
    ...
    /// This should be called after checkpoint stats and worker job events are
    /// reported
    pub async fn calculate_and_distribute_rewards(&self, checkpoint_id: i64) -> anyhow::Result<Vec<CheckpointRewardDistribution>> {
        info!("Starting reward calculation for checkpoint {}", checkpoint_id);

        // 1. Get checkpoint stats
        let checkpoint_stats = CheckpointStatsRepository::get_by_checkpoint_id(&self.pool, checkpoint_id).await?;
        ...
        if checkpoint_stats.fees_collected == 0 {
            info!("No fees collected at checkpoint {}, skipping reward distribution", checkpoint_id);
            return Ok(vec![]);
        }
        ...
        if job_events.is_empty() {
            tracing::warn!("No job events found for checkpoint {} despite fees being collected", checkpoint_id);
            return Ok(vec![]);
        }
        ...
        info!(
            ...
            checkpoint_id, total_fees, total_jobs, reward_per_job, remainder
        );
        ...
        let created_distributions = CheckpointRewardDistributionRepository::create_batch(&self.pool, &distributions).await?;

        // 6. Log summary by worker
        let mut worker_rewards: HashMap<String, (i64, i64)> = HashMap::new();
        for dist in &created_distributions {
            let entry = worker_rewards.entry(dist.worker_public_key.clone()).or_insert((0, 0));
            entry.0 += 1; // job count
        ...
        for (worker_public_key, (job_count, total_reward)) in worker_rewards {
            info!(
                ...
                worker_public_key, job_count, total_reward, job_count
            );
        }
    ...
    pub async fn get_checkpoint_summary(&self, checkpoint_id: i64) -> anyhow::Result<Option<CheckpointRewardSummary>> {
        CheckpointRewardDistributionRepository::get_checkpoint_summary(&self.pool, checkpoint_id).await
    }

    /// Get worker's reward statistics
    pub async fn get_worker_stats(&self, worker_public_key: &str) -> anyhow::Result<Option<WorkerCheckpointRewardStats>> {
    ...
    pub async fn get_worker_rewards_aggregated(
        ...
        CheckpointRewardAggregationRepository::get_aggregations(&self.pool, view_name, worker_public_key, start_time, end_time, limit).await
    }

    /// Start background task to process pending checkpoint rewards
    /// This runs periodically and checks for checkpoints that need reward
    /// calculation
    pub async fn start_checkpoint_reward_task(pool: PgPool, interval_seconds: u64) {
        info!("Starting checkpoint reward processing task (interval: {}s)", interval_seconds);
            ...
            match Self::process_pending_checkpoint_rewards(&pool).await {
                Ok(processed_count) => {
                    if processed_count > 0 {
                        info!("✅ Successfully processed rewards for {} checkpoints", processed_count);
                    }
                ...
                Err(e) => {
                    ...
                    if consecutive_errors >= MAX_CONSECUTIVE_ERRORS {
                        tracing::error!("🚨 Too many consecutive errors ({}), backing off for 5 minutes", consecutive_errors);
    ...
    async fn process_pending_checkpoint_rewards(pool: &PgPool) -> anyhow::Result<usize> {
        ...
        let (valid_checkpoints, invalid_checkpoints): (Vec<i64>, Vec<i64>) = pending_checkpoints.into_iter().partition(|&id| id >= 0);

        if !invalid_checkpoints.is_empty() {
            warn!("⚠️ Found invalid (negative) checkpoint IDs in DB: {:?}", invalid_checkpoints);
        }
            ...
            match Self::calculate_and_distribute_rewards_static(pool, checkpoint_id).await {
                Ok(distributions) => {
                    if !distributions.is_empty() {
                        ...
                        info!("⏭️  Skipped checkpoint {} (no fees to distribute)", checkpoint_id);
                    }
                }
                Err(e) => {
                    tracing::error!("❌ Failed to process checkpoint {}: {}", checkpoint_id, e);
    ...
    /// Find checkpoints that have both stats and job events but no reward
    /// distributions
    async fn find_pending_checkpoints(pool: &PgPool) -> anyhow::Result<Vec<i64>> {
        ...
        let rows = sqlx::query_scalar::<_, i64>(query).fetch_all(pool).await?;
    ...
    pub async fn calculate_and_distribute_rewards_static(pool: &PgPool, checkpoint_id: i64) -> anyhow::Result<Vec<CheckpointRewardDistribution>> {
        info!("Starting reward calculation for checkpoint {}", checkpoint_id);

        // 1. Get checkpoint stats
        let checkpoint_stats = CheckpointStatsRepository::get_by_checkpoint_id(pool, checkpoint_id).await?;
        ...
        if job_events.is_empty() {
            warn!("❗ No job events found for checkpoint {} despite fees being collected", checkpoint_id);
            return Ok(vec![]);
        }
        ...
        if total_fees <= 0 {
            error!("❌ Checkpoint {} has non-positive fees: {}, skipping", checkpoint_id, total_fees);
            return Ok(vec![]);
        }
        ...
        // 5. Batch insert reward distributions (one per job)
        let created_distributions = CheckpointRewardDistributionRepository::create_batch(pool, &distributions).await?;
