...
use crate::{
    ...
    repositories::WorkerRewardsAggregationRepository,
    Result,
};
...
impl WorkerStatsRepository {
    ...
    pub async fn get_worker_stats(pool: &PgPool, worker_public_key: &str) -> Result<WorkerStats> {
        ...
        // Get PROCESSING and PENDING tasks count from latest_job_status materialized
        // view This gives accurate current state without double-counting from
        // worker_events audit trail
        ...
        let total_rewards = WorkerRewardsAggregationRepository::get_worker_rewards_all(pool, worker_public_key)
            .await
            .unwrap_or(0);

        // Get completed and failed counts for different time windows from
        // latest_job_status Use worker_events for historical time-based queries
        // since latest_job_status only has current state
        ...
        let last_completed_block_height = last_block_row.and_then(|row| Some(row.checkpoint_id));
