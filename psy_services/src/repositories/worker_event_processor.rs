...
impl WorkerEventProcessor {
    ...
    async fn handle_rollback(
        ...
        let selected_slot = *slot_ids.iter().max().ok_or_else(|| anyhow::anyhow!("No slots found"))?;
        ...
        let selected_events = events_by_slot
            .into_iter()
    ...
    fn group_events_by_slot(&self, events: &[WorkerEventForProcessing]) -> HashMap<i64, Vec<WorkerEventForProcessing>> {
        ...
        for event in events {
            ...
            grouped.entry(slot_id).or_insert_with(Vec::new).push(event.clone());
        }
    ...
    async fn insert_job_events(&self, checkpoint_id: i64, events: &[WorkerEventForProcessing]) -> Result<usize> {
        ...
        let existing_set: std::collections::HashSet<String> = existing_job_ids.into_iter().map(|s| s.to_lowercase()).collect();

        for batch in events.chunks(BATCH_SIZE) {
            for event in batch {
                // Extract worker public key (required field)
                let worker_public_key = event.public_key.clone().unwrap_or_else(|| "unknown".to_string());

                // Extract topic and circuit_type from metadata or job_id
                let topic = event.metadata.as_ref().and_then(|m| m.get("topic")).and_then(|v| v.as_i64()).unwrap_or(0) as i16;

                let circuit_type = event
                    .metadata
                    .as_ref()
                ...
                if existing_set.contains(&job_id_str) {
                    debug!("Skipping duplicate job_id for checkpoint {}: {}", checkpoint_id, job_id_str);
                    continue;
                }
