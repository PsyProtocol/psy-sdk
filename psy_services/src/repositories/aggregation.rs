...
impl WorkerRewardsAggregationRepository {
    ...
    /// These aggregates now query checkpoint_reward_distributions for actual
    /// rewards
    pub async fn get_aggregations(
        pool: &PgPool,
        view_name: &str,
        ...
        let valid_views = vec!["worker_rewards_1d", "worker_rewards_1w", "worker_rewards_1m", "worker_rewards"];
        ...
        let aggregations = if view_name == "worker_rewards" {
            ...
            tracing::debug!("Querying worker_rewards table for worker {}", worker_public_key);
    ...
    /// Note: worker_rewards table is updated via trigger, so no need to manual
    /// refresh
    ...
    pub async fn get_worker_rewards_all(pool: &PgPool, worker_public_key: &str) -> Result<i64> {
        let query = r#"
        ...
        let row = sqlx::query(query).bind(worker_public_key).fetch_optional(pool).await?;
