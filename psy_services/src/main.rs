...
/// Psy API Service - Zero Knowledge Proof Backend
#[derive(Parser, Debug)]
#[command(
    name = "psy-api",
    version,
    about = "Run the psy API Service with configurable parameters",
    long_about = None
)]
struct Cli {
...
async fn main() -> anyhow::Result<()> {
    ...
    config.server = psy_services::config::ServerConfig {
        host: cli.listen_addr.clone(),
        port: cli.port,
    };

    config.database = psy_services::config::DatabaseConfig {
        url: cli.database_url.clone(),
        max_connections: cli.max_connections,
    };

    tracing::info!(
        "Starting psy API server at {}:{} (DB: {}, max_conn={})",
        cli.listen_addr,
        cli.port,
        cli.database_url,
        cli.max_connections
