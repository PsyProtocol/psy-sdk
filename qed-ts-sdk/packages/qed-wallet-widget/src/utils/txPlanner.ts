...
function getStandardP2PKHTxSize(
    numberOfInputs: number,
    numberOfP2PKHOutputs: number,
    numberOfP2SHOutputs: number
): number {
    const base = 8;
    const headerSize =
        base +
        varuintEncodingLength(numberOfInputs) +
        varuintEncodingLength(numberOfP2PKHOutputs + numberOfP2SHOutputs);
    const inputsSize = numberOfInputs * (40 + P2PKH_INPUT_SCRIPT_LENGTH);
    const outputsSize =
        numberOfP2PKHOutputs * (8 + P2PKH_OUTPUT_SCRIPT_LENGTH) + numberOfP2SHOutputs * (8 + P2SH_OUTPUT_SCRIPT_LENGTH);
    return headerSize + inputsSize + outputsSize;
}
...
function coinSelect(settings: ITxPlanSettings): IUTXO[] {
    ...
    function createSolution(): { utxos: IUTXO[]; fee: number; change: number[] } | null {
        ...
        for (const utxo of shuffledUTXOs) {
            ...
            fee =
                getTxSize(inputScriptLength, selectedUTXOs.length, [
                    ...outputs.map((o) => o.scriptLength),
                    ...changeOutputs.map(() => outputSelfScriptLength),
                ]) * feeRate;
...
function coinSelectP2PKH(
    address: string,
    feeRate: number,
    utxos: IUTXO[],
    outputs: ITransactionOutputAddress[]
): ICreateP2PKHParams & { fee: number } {
