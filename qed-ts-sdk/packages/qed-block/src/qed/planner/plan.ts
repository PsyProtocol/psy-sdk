...
function planJobs(
    ...
    const root_state_transition = blockStateTransitionInputWitnessJobId(checkpoint_id);

    const agg_jobs_for_inputs = Array.from({ length: num_input_witnesses }, (v, i) =>
    ...
    const per_input_jobs = Array.from({ length: num_input_witnesses }, (v, i) => {
        return [
    ...
    const agg_state_and_introspections_group_id = 5;
    const agg_state_root_id = getBlockAggregateJobsGroupJobId(checkpoint_id, agg_state_and_introspections_group_id, 0);
    const agg_all_introspections_ids = getBlockAggregateJobsGroupJobId(
        checkpoint_id,
        agg_state_and_introspections_group_id,
        1
    );
    const introspection_jobs = per_input_jobs.map((x) => x[2]);
    proof_store.write_next_jobs(introspection_jobs, [agg_all_introspections_ids]);
    const final_input_witness_jobs = per_input_jobs.map((x) => x[1]);
    ...
    const op_agg_group_parts_common_id = 6;

    const state_part_1_common_id = getBlockAggregateJobsGroupJobId(checkpoint_id, op_agg_group_parts_common_id, 0);
    const state_part_2_common_id = getBlockAggregateJobsGroupJobId(checkpoint_id, op_agg_group_parts_common_id, 1);

    const state_part_1_id = blockAggStatePart1InputWitnessJobId(checkpoint_id);
    const state_part_2_id = blockAggStatePart2InputWitnessJobId(checkpoint_id);
    ...
    const op_agg_group_part_1_id = 11;
    const register_users_agg_job_id = getBlockAggregateJobsGroupJobId(checkpoint_id, op_agg_group_part_1_id, 0);
    const claim_deposits_agg_job_id = getBlockAggregateJobsGroupJobId(checkpoint_id, op_agg_group_part_1_id, 1);
    const transfer_tokens_agg_job_id = getBlockAggregateJobsGroupJobId(checkpoint_id, op_agg_group_part_1_id, 2);
    ...
    const op_agg_group_part_2_id = 12;
    const add_withdrawals_agg_job_id = getBlockAggregateJobsGroupJobId(checkpoint_id, op_agg_group_part_2_id, 0);
    const process_withdrawals_agg_job_id = getBlockAggregateJobsGroupJobId(checkpoint_id, op_agg_group_part_2_id, 1);
    const add_deposits_agg_job_id = getBlockAggregateJobsGroupJobId(checkpoint_id, op_agg_group_part_2_id, 2);
    ...
    const leaf_jobs = introspection_jobs
