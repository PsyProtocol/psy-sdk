...
use psy_services::{
    ...
    models::{
        CheckpointLeafStat, CheckpointLeavesRequest, CheckpointLeavesResponse, UserEvent, UserEventTxType, WorkerEvent, WorkerEventSource,
        WorkerEventStatus,
    },
};
use reqwest::Client;
...
impl ApiClientConfig {
    pub fn new(endpoint: String, node_id: String, node_type: WatcherSourceNodeType, realm_id: Option<i64>) -> Self {
...
impl ApiClient {
    pub fn new(endpoint: String, node_id: String, node_type: WatcherSourceNodeType, realm_id: Option<i64>) -> Result<Self> {
    ...
    fn generate_jwt_token(&self) -> Result<String> {
        ...
        let token = encode(&Header::default(), &claims, &EncodingKey::from_secret(jwt_secret.as_bytes())).context("Failed to generate JWT token")?;

        debug!("Generated JWT token for watcher-{} (expires at {})", self.config.node_id, exp);
    ...
    pub(crate) async fn get_headers(&self, path: &str) -> Result<reqwest::header::HeaderMap> {
        ...
        if path.starts_with("/telemetry/") {
            ...
            if let Some(token_str) = &*token {
                headers.insert(
                    "Authorization",
                    format!("Bearer {}", token_str).parse().context("Failed to create Authorization header")?,
                );
            } else {
    ...
    pub async fn send_user_registration(&self, event: UserRegistrationEvent) -> Result<()> {
        let short_key = event.metadata.public_key.get(..8).unwrap_or(&event.metadata.public_key);
    ...
    pub async fn send_contract_deployment(&self, event: UserDeployContractEvent) -> Result<()> {
        let short_key = event.deployer.get(..8).unwrap_or(&event.deployer);
    ...
    pub async fn send_endcap_submission(&self, event: UserEndcapSubmissionEvent) -> Result<()> {
        ...
        self.send_user_events(vec![api_event]).await?;
        debug!(
            "UserEndcap submission event sent for realm_id: {}, user_id: {}",
            event.realm_id, event.user_id
        );
        Ok(())
    }
    ...
    pub async fn send_checkpoint_leaves(&self, checkpoint_leaves: Vec<CheckpointLeafWithId>) -> Result<()> {
        for batch in checkpoint_leaves.chunks(MAX_BATCH_SIZE) {
            ...
            self.send_telemetry_request::<CheckpointLeavesRequest, CheckpointLeavesResponse>("/telemetry/checkpoint/leaves", &request)
                .await?;
        }
    ...
    async fn send_telemetry(&self, payload: &TelemetryPayload) -> Result<()> {
        let _response: TelemetryResponse = self.send_telemetry_request("/telemetry/events", payload).await?;
    ...
    pub(crate) async fn send_telemetry_request<T: Serialize, R: for<'de> Deserialize<'de>>(&self, path: &str, payload: &T) -> Result<R> {
        let url = format!("{}{}", self.config.endpoint, path);
        let headers = self.get_headers(path).await?;

        let response = self.client.post(&url).headers(headers).json(payload).send().await?;
        ...
        match response.status() {
            ...
            status if !status.is_success() => Err(anyhow::anyhow!("Request failed with status: {}", status)),
    ...
    async fn send_with_retry<T, R>(&self, method: &str, path: &str, body: Option<&T>) -> Result<R>
    where
        T: Serialize + ?Sized,
    ...
    pub async fn health_check(&self) -> Result<bool> {
        match self.send_with_retry::<(), serde_json::Value>("GET", "/health", None).await {
            Ok(response) => Ok(response.get("status").and_then(|s| s.as_str()).map_or(false, |s| s == "ok")),
            Err(e) => {
