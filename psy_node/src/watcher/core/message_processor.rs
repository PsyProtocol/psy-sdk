...
use crate::{
    ...
    watcher::{
        constant::{
            FAILURE_BACKOFF_DURATION, MAX_CONCURRENT_TASKS, MAX_RETRY_ATTEMPTS, MAX_SINGLE_MESSAGE_PROCESSING_TIME, RETRY_ATTEMPT_TTL_SECS,
            SLEEP_TIME_IF_HAVE_MSG, SLEEP_TIME_IF_NO_MSG,
        },
        contract_monitor::ContractMonitorHandle,
        error::WatcherError,
...
impl MessageProcessorHandle {
    ...
    async fn handle_message(&self, message: &WatcherMessage) -> Result<(), WatcherError> {
        ...
        match message {
            UserRegistration(event) => {
                ...
                self.api_client
                    .send_user_registration(event.clone())
                    .await
            ...
            DeployContract(event) => {
                ...
                self.api_client
                    .send_contract_deployment(event.clone())
                    .await
                    .map_err(|e| WatcherError::ApiClient(e.to_string()))?;
            ...
            GutaSubmission(event) => {
                info!("UserEvent: GUTA realm={} circuit={}", event.realm_id, event.metadata.circuit_type);
                self.api_client
                    .send_guta_submission(event.clone())
                    .await
                    .map_err(|e| WatcherError::ApiClient(e.to_string()))
            }
            EndcapSubmission(event) => {
                ...
                self.api_client
                    .send_endcap_submission(event.clone())
                    .await
                    .map_err(|e| WatcherError::ApiClient(e.to_string()))
            }
            JobPending(event) => {
            ...
            JobCompleted(event) => {
                ...
                self.api_client
                    .send_job_completed(event.clone())
                    .await
                    .map_err(|e| WatcherError::ApiClient(e.to_string()))
            }
            JobTimeout(event) => {
    ...
    async fn handle_processing_failure(&self, msg_id: &str, message: &WatcherMessage, attempts: u32, error: WatcherError) {
    ...
    async fn send_to_dead_letter_queue(&self, msg_id: &str, message: &WatcherMessage, error: WatcherError) {
    ...
    async fn get_message_attempts(&self, msg_id: &str) -> Option<u32> {
        ...
        self.redis_pool.get().await.ok()?.get(&key).await.ok()
    }
    ...
    async fn increment_message_attempts(&self, msg_id: &str) {
        ...
        if let Ok(mut conn) = self.redis_pool.get().await {
            ...
            let _: std::result::Result<(), redis::RedisError> = conn.set_ex(&key, attempts, RETRY_ATTEMPT_TTL_SECS).await;
        }
    }
