...
use crate::{
    common::retry::{retry_with_backoff, RetryConfig},
    watcher::{constant::BLOCK_METADATA_FINALIZATION_DELAY, error::WatcherError, ApiClient},
};

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CheckpointLeafWithId {
    pub checkpoint_id: u64,
    pub checkpoint_leaf: PsyCheckpointLeaf<PsyFelt>,
}
...
impl CheckpointSenderService {
    pub fn new(psy_store: Arc<PsyStore>, api_client: Arc<ApiClient>, latest_height: Arc<AtomicU64>) -> Self {
        Self {
            psy_store,
            api_client,
            latest_height,
    ...
    async fn process_checkpoints(&self) -> Result<()> {
        ...
        debug!(
            "🆕 CheckpointSenderService: latest_height={}, finalized_height={}, local_height={}",
        ...
        self.api_client
            .send_checkpoint_leaves(checkpoint_leaves)
            .await
        ...
        info!("Successfully sent checkpoints {} to {}", local_height, finalized_height);
    ...
    async fn fetch_checkpoint_range(&self, start: u64, end: u64) -> Result<Vec<CheckpointLeafWithId>> {
        ...
        for checkpoint_id in start..end {
            match self.fetch_single_checkpoint(checkpoint_id).await {
                ...
                Err(e) => {
                    error!("Failed to fetch checkpoint {} after retries: {}. Aborting this batch.", checkpoint_id, e);
                    return Err(e);
                }
            }
        }
    ...
    async fn fetch_single_checkpoint(&self, checkpoint_id: u64) -> Result<PsyCheckpointLeaf<PsyFelt>> {
        let config = RetryConfig::default();

        retry_with_backoff(&config, &format!("fetch checkpoint {}", checkpoint_id), || async {
            PsyCoordinatorStoreReaderAsync::get_checkpoint_leaf_data(&self.psy_store, checkpoint_id)
                .await
