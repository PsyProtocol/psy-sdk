...
pub struct RealmRecoveryManager {
    ...
    sync_queue: std::sync::Arc<ProofStoreRedis>,
    config_path: String,
}
...
impl RealmRecoveryManager {
    pub async fn new(
        ...
        let sync_queue = std::sync::Arc::new(ProofStoreRedis::new(&redis_uri, queue_biz_key).await?);
        ...
        for checkpoint_id in 0..=target {
            ...
            match self.recover_checkpoint(checkpoint_id).await {
                Ok(()) => {
                    ...
                    self.store.commit(None).await?; // Commit journal store
                }
                Err(e) => {
