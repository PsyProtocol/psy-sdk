...
var numberParsers = require('./numberParsers.cjs');
var revive = require('./revive.cjs');
...
function parse(text, reviver, options) {
  ...
  const parseNumber = optionsObj?.parseNumber ?? numberParsers.parseLosslessNumber;
  const onDuplicateKey = optionsObj?.onDuplicateKey ?? throwDuplicateKey;
  let i = 0;
  const value = parseValue();
  expectValue(value);
  expectEndOfInput();
  return reviver ? revive.revive(value, reviver) : value;
