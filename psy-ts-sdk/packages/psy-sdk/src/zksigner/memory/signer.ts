...
import { ClaimBatchItem, ContractCallArgs, ContractCallData, DPNFunctionCircuitDefinition, IPsyUserProverProvider, SignType, TxMetadata } from "../../local-prover-rpc";
...
class PsyMemoryTransactionSigner implements IPsyTransactionSigner {
    ...
    async getSigHash(pk_hash: string, callData: ContractCallData): Promise<string> {
        return this.prover.getSigHash(pk_hash, callData);
    }

    async execContractCallWithExternalSignature(pk_hash: string, callData: ContractCallData, signatureHex: string): Promise<string> {
        return this.prover.execContractCallWithExternalSignature(pk_hash, callData, signatureHex);
    }
    ...
    async registerUserWithExternalSignature(publicKeyHex: string, signatureHex: string): Promise<{ pk_hash: string; user_id: string | null }> {
        return this.prover.registerUserWithExternalSignature(publicKeyHex, signatureHex);
    }

    async getClaimSigHash(pk_hash: string, claims: ClaimBatchItem[]): Promise<string> {
        return this.prover.getClaimSigHash(pk_hash, claims);
    }

    async claimBatchWithExternalSignature(pk_hash: string, claims: ClaimBatchItem[], signatureHex: string): Promise<string> {
        return this.prover.claimBatchWithExternalSignature(pk_hash, claims, signatureHex);
    }
}
