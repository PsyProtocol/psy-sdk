...
type TEventListenerMap<T extends IndexableType, TEvents extends IBaseEvent<T>> = {
    [K in T]?: ((event: TEvents & { type: K }) => any)[];
};
...
class EventHub<TEventType extends IndexableType, TEvents extends IBaseEvent<TEventType>> {
    ...
    addEventListener<T extends TEventType>(type: T, listener: (event: TEvents & { type: T }) => any): boolean {
    ...
    on<T extends TEventType>(type: T, listener: (event: TEvents & { type: T }) => any): boolean {
    ...
    remove<T extends TEventType>(type: T, listener: (event: TEvents & { type: T }) => any): boolean {
        return this.removeEventListener(type, listener);
    }
    once<T extends TEventType>(type: T, listener: (event: TEvents & { type: T }) => any): () => boolean {
        let called = false;
    ...
    removeEventListener<T extends TEventType>(type: T, listener: (event: TEvents & { type: T }) => any): boolean {
    ...
    notifyWithErrors<T extends TEventType>(
        eventOrType: T | (TEvents & { type: T }),
        event?: Omit<TEvents & { type: T }, "type">
    ): any[] {
