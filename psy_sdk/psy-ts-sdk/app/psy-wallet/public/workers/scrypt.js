...
const HOST_MESSAGE_TYPES = {Scrypt: "Scrypt"};
const WORKER_MESSAGE_TYPES = {
  ScryptResult: "ScryptResult",
  ScryptError: "ScryptError",
};
function runScrypt({n, r, p, dkLen, password, salt}){
  return lib.syncScrypt(hexToU8Array(password), hexToU8Array(salt), n, r, p, dkLen);
}
...
function sendResult(id, data){
  postMessage({id: id, type: WORKER_MESSAGE_TYPES.ScryptResult, data: u8ArrayToHex(data)});
}
function sendError(id, data){
  postMessage({id: id, type: WORKER_MESSAGE_TYPES.ScryptError, data});
}

onmessage = function (event) {
  const {id, type, data} = event.data;
  if (type === HOST_MESSAGE_TYPES.Scrypt) {
    try {
      const result = runScrypt(data);
