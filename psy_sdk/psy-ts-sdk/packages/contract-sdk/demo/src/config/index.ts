...
export const config = {
    ...
    user: {
        ...
        get currentKeyPair(): KeyPairInfo {
            ...
            const matchingPair = Object.values(config.user.keyPairs).find((kp) => kp.privateKey === currentPrivateKey);
    ...
    logging: {
        ...
        rustLogLevel:
            ...
            "psy_user_cli=debug,psy_dev_cli=debug,psy_node_cli=debug,psy_node=debug,psy_common_circuit=debug,psy_network_circuit=debug,psy_prover=debug,psy_data=debug,plonky2=error",
    },
};

// Helper to get Realm RPC URL based on environment and realm ID
export function getRealmRpcUrl(environment: "local" | "testnet" | "mainnet" = "local", realmId?: number): string {
...
export function getCoordinatorRpcUrl(environment: "local" | "testnet" | "mainnet" = "local"): string {
