    ...
    for Formatter<'a, F, C>
{
    ...
    fn visit_value(
        ...
        Ok(match node {
            ...
            ValueNode::Struct(name, generic_parameters, field_values, Span) => {
    ...
    fn visit_return(
        ...
        let ReturnNode { expr_id, ref span } = ctx.statement(stmt_id).as_return().unwrap();
        let s = format!(
            "return{};",
    ...
    fn visit_match(
        ...
        let MatchNode {
            ...
            arms,
            span,
        } = ctx.statement(node).as_match().cloned().unwrap();
