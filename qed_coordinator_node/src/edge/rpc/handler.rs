...
use crate::edge::context::{
    with_ctx_read_async, GLOBAL_COORD_EDGE_CTX, LATEST_CHECKPOINT_ID, REGISTERED_USERS,
...
use qed_node::nimpl::worker_queue_redis::redis_queue::{
    CEQueueNotification, CPQueueNotification, RedisQueue, CE_NOTIFICATIONS, CP_NOTIFICATIONS,
};
...
impl CoordinatorEdgeHandler {
                                ...
                                if jobs.is_empty() {
                                    ...
                                    for job_id in jobs {
                                        ...
                                        if let Err(e) = process_realm_job(ctx, job_id).await {
                                            ...
                                            info!(
                                                "✅ process_realm_job success, checkpoint = {}",
    ...
    pub async fn get_user_id_by_pub_key(
        &self,
    ...
    pub async fn submit_guta(
        ...
        info!("Pushing job id to queue: {:?}", input);
