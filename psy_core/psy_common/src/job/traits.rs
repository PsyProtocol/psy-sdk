...
pub trait QProofStoreReaderSync {
    ...
    fn get_next_jobs_by_job_id(&self, id: QProvingJobDataID) -> anyhow::Result<Vec<QProvingJobDataID>> {
        let counter_id = id.get_sub_group_counter_id();
...
pub trait QProofStoreWriterSync {
    ...
    fn write_next_jobs_core(&mut self, jobs: &[QProvingJobDataID], next_jobs: &[QProvingJobDataID]) -> anyhow::Result<()> {
    ...
    fn write_multidimensional_jobs_core(&mut self, jobs_levels: &[Vec<QProvingJobDataID>], next_jobs: &[QProvingJobDataID]) -> anyhow::Result<()> {
        ...
        for i in 0..job_levels_count {
            ...
            self.set_bytes_by_id(
                next_jobs_id,
                &bincode::serialize(if i == (job_levels_count - 1) { next_jobs } else { &jobs_levels[i + 1] })?,
            )?;
        }
        Ok(())
    }
...
pub trait QProofStoreWriterSyncImm {
    ...
    fn write_next_jobs_imm(&self, jobs: &[QProvingJobDataID], next_jobs: &[QProvingJobDataID]) -> anyhow::Result<()>;
    fn write_next_jobs_core_imm(&self, jobs: &[QProvingJobDataID], next_jobs: &[QProvingJobDataID]) -> anyhow::Result<()> {
        let counter_id = jobs[0].get_sub_group_counter_id();
        let goal_id = counter_id.get_sub_group_counter_goal_id();
        let next_jobs_id = counter_id.get_sub_group_counter_goal_next_jobs_id();
    ...
    fn write_multidimensional_jobs_imm(&self, jobs_levels: &[Vec<QProvingJobDataID>], next_jobs: &[QProvingJobDataID]) -> anyhow::Result<()>;
    fn write_multidimensional_jobs_core_imm(&self, jobs_levels: &[Vec<QProvingJobDataID>], next_jobs: &[QProvingJobDataID]) -> anyhow::Result<()> {
        let job_levels_count = jobs_levels.len();
        for i in 0..job_levels_count {
            let counter_id = jobs_levels[i][0].get_sub_group_counter_id();
            let goal_id = counter_id.get_sub_group_counter_goal_id();
...
pub trait QProofStoreReaderAsync {
    ...
    async fn get_next_jobs_by_job_id(&self, id: QProvingJobDataID) -> anyhow::Result<Vec<QProvingJobDataID>> {
        let counter_id = id.get_sub_group_counter_id();
        let next_jobs_id = counter_id.get_sub_group_counter_goal_next_jobs_id();
...
pub trait QProofStoreWriterAsyncImm: Send + Sync {
    ...
    async fn write_next_jobs(&self, jobs: &[QProvingJobDataID], next_jobs: &[QProvingJobDataID]) -> anyhow::Result<()>;
    async fn write_next_jobs_core(&self, jobs: &[QProvingJobDataID], next_jobs: &[QProvingJobDataID]) -> anyhow::Result<()> {
        let counter_id = jobs[0].get_sub_group_counter_id();
        let goal_id = counter_id.get_sub_group_counter_goal_id();
        let next_jobs_id = counter_id.get_sub_group_counter_goal_next_jobs_id();
    ...
    async fn write_multidimensional_jobs(&self, jobs_levels: &[Vec<QProvingJobDataID>], next_jobs: &[QProvingJobDataID]) -> anyhow::Result<()>;
    async fn write_multidimensional_jobs_core(&self, jobs_levels: &[Vec<QProvingJobDataID>], next_jobs: &[QProvingJobDataID]) -> anyhow::Result<()> {
        let job_levels_count = jobs_levels.len();
        for i in 0..job_levels_count {
            let counter_id = jobs_levels[i][0].get_sub_group_counter_id();
            let goal_id = counter_id.get_sub_group_counter_goal_id();
...
#[async_trait]
pub trait QWorkerGenericProverAsyncMut<S: QProofStoreReaderAsync, C: GenericConfig<D>, const D: usize>: QWorkerVerifyHelper<C, D> {
    async fn worker_prove_mut(&mut self, store: &S, job_id: QProvingJobDataID) -> anyhow::Result<ProofWithPublicInputs<C::F, C, D>>;
}

pub trait QWorkerGenericProverMut<S: QProofStoreReaderSync, C: GenericConfig<D>, const D: usize>: QWorkerVerifyHelper<C, D> {
    fn worker_prove_mut(&mut self, store: &S, job_id: QProvingJobDataID) -> anyhow::Result<ProofWithPublicInputs<C::F, C, D>>;
}
...
>
{
    fn prove_q_worker_simple(&self, verify_helper: &V, store: &S, job_id: QProvingJobDataID) -> anyhow::Result<ProofWithPublicInputs<C::F, C, D>>;
}
...
>
{
    ...
    fn prove_q_worker_standard(&self, verify_helper: &V, store: &S, job_id: QProvingJobDataID) -> anyhow::Result<ProofWithPublicInputs<C::F, C, D>> {
...
>
{
    fn prove_q_worker_agg(&self, verify_helper: &V, store: &S, job_id: QProvingJobDataID) -> anyhow::Result<ProofWithPublicInputs<C::F, C, D>>;
}

pub trait QWorkerCircuitCustomWithDataSync<V: QWorkerVerifyHelper<C, D>, S: QProofStoreReaderSync, C: GenericConfig<D>, const D: usize> {
    fn prove_q_worker_custom(&self, verify_helper: &V, store: &S, job_id: QProvingJobDataID) -> anyhow::Result<ProofWithPublicInputs<C::F, C, D>>;
}
pub trait QWorkerCircuitCompressWithDataSync<S: QProofStoreReaderSync> {
    fn prove_q_worker_compress(&self, store: &S, job_id: QProvingJobDataID) -> anyhow::Result<String>;
}
pub trait QWorkerCircuitMutCustomWithDataSync<S: QProofStoreReaderSync, C: GenericConfig<D>, const D: usize> {
    fn prove_q_worker_mut_custom(&mut self, store: &S, job_id: QProvingJobDataID) -> anyhow::Result<ProofWithPublicInputs<C::F, C, D>>;
}
