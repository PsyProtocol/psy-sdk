...
impl<F: RichField> AggStateTransition<F> {
    ...
    pub fn get_combined_hash<H: AlgebraicHasher<F>>(&self) -> QHashOut<F> {
        QHashOut(H::two_to_one(self.state_transition_start.0, self.state_transition_end.0))
    }
}
...
impl<F: RichField> AggStateTransitionWithEventsInput<F> {
    pub fn condense(&self) -> AggStateTransitionWithEvents<F> {
        AggStateTransitionWithEvents {
            state_transition_start: self.left_input.state_transition_start,
            state_transition_end: self.right_input.state_transition_end,
            event_hash: QHashOut(PoseidonHash::two_to_one(self.left_input.event_hash.0, self.right_input.event_hash.0)),
        }
    }
    pub fn combine_with_right_leaf<T: AggStateTrackableWithEventsInput<F>>(&self, right: &T) -> Self {
        Self {
            left_input: self.condense(),
...
impl<IL: AggStateTrackableInput<F>, F: RichField> TPLeafAggregator<IL, AggStateTransitionInput<F>> for AggWTLeafAggregator {
    fn get_output_from_inputs(left: &AggStateTransitionInput<F>, right: &AggStateTransitionInput<F>) -> AggStateTransitionInput<F> {
    ...
    fn get_output_from_left_leaf(left: &IL, right: &AggStateTransitionInput<F>) -> AggStateTransitionInput<F> {
        right.combine_with_left_leaf(left)
    }

    fn get_output_from_right_leaf(left: &AggStateTransitionInput<F>, right: &IL) -> AggStateTransitionInput<F> {
...
impl<IL: AggStateTrackableWithEventsInput<F>, F: RichField> TPLeafAggregator<IL, AggStateTransitionWithEventsInput<F>> for AggWTTELeafAggregator {
    ...
    fn get_output_from_left_leaf(left: &IL, right: &AggStateTransitionWithEventsInput<F>) -> AggStateTransitionWithEventsInput<F> {
        right.combine_with_left_leaf(left)
    }

    fn get_output_from_right_leaf(left: &AggStateTransitionWithEventsInput<F>, right: &IL) -> AggStateTransitionWithEventsInput<F> {
        left.combine_with_right_leaf(right)
    }
...
impl<F: RichField> TPCircuitFingerprintConfig<F> {
    pub fn from_leaf_and_agg_fingerprints<H: AlgebraicHasher<F>>(
        ...
        let allowed_circuit_hashes_root = QHashOut(H::two_to_one(leaf_fingerprint.0, aggregator_fingerprint.0));
        Self {
            leaf_fingerprint,
...
pub fn generate_tree_inputs_with_position<LA: TPLeafAggregator<IL, IO>, IL: Clone, IO: Clone>(leaf_inputs: &[IL]) -> Vec<Vec<TreeAggJob<IO>>> {
...
pub fn generate_tree_inputs_from_leaves<LA: TPLeafAggregator<IL, IO>, IL: Clone, IO: Clone>(leaf_inputs: &[IL]) -> Vec<Vec<IO>> {
    let tree_positions = BinaryTreePlanner::new(leaf_inputs.len()).levels;
