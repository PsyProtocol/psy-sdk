...
class CityRPCProvider implements ICityRPCProvider {
  ...
  async getCityBlockDepositAddressString(
    checkpoint_id: number
  ): Promise<string> {
  ...
  async getWithdrawalHash(
    checkpoint_id: number,
    withdrawal_id: number
  ): Promise<CityHash> {
    return this.rpc("cr_getWithdrawalHash", [checkpoint_id, withdrawal_id]);
  }
  async getWithdrawalLeafMerkleProof(
    checkpoint_id: number,
    withdrawal_id: number
  ): Promise<CityMerkleProof> {
    return this.rpc("cr_getWithdrawalLeafMerkleProof", [
      checkpoint_id,
      withdrawal_id,
    ]);
  }
