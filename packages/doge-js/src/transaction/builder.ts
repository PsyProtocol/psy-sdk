...
import { getP2SHOutputScript, isP2PKHOutputScript } from "../address";
...
async function defaultFinalizer(info: IFinalizerInfo): Promise<Uint8Array> {
  console.log("info.sigHashPreimage", u8ArrayToHex(info.sigHashPreimage));
  console.log("info.sigHash", u8ArrayToHex(info.sigHash));
  ...
  async finalizeAndSign(): Promise<Transaction> {
    ...
    for (let i = 0; i < this.inputs.length; i++) {
      ...
      if (Array.isArray(signers)) {
        ...
        if (
          signers.length === 1 &&
          isP2PKHOutputScript(this.inputs[i].lockScript)
        ) {
          const pubKeyHash = hashBuffer("hash160", newSignatures[0].pubkey);
          if (
            u8ArrayToHex(pubKeyHash) ===
            u8ArrayToHex(this.inputs[i].lockScript.slice(3, 23))
          ) {
            isP2PKH = true;
          }
        }
      }
      if (!isP2PKH && isP2PKHOutputScript(this.inputs[i].lockScript)) {
        if (unlockScript.length === 0) {
          throw new Error("P2PKH script requires a solution or a signer");
        } else {
          // get the public key from the last item in the script solution
          let lastItem = unlockScript
            .map((x) => disassembleScript(x))
            .join(" ")
            .split(" ")
            .map((x) => x.trim())
            .filter((x) => x.length)
            .pop();
          if (!lastItem) {
            throw new Error("Invalid script solution");
          }
          const pubKey = hexToU8Array(lastItem);
          const pubKeyHash = hashBuffer("hash160", pubKey);
          if (
            u8ArrayToHex(pubKeyHash) !==
            u8ArrayToHex(this.inputs[i].lockScript.slice(3, 23))
          ) {
