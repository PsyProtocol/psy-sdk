...
class PSCityBlock<T> {
  ...
  constructor(rpc: ICityRPCProvider, dogeRPC: IDogeLinkElectrsRPC, synthResult: ICitySynthBlockResult, dumpConfig: IDumpProofStoreConfig, simpleBlock: ISimpleCityBlock, loadJobInfoHelper: (ctx: PSCityBlock<T>, jobId: string) => Promise<T>, blockExplorerUrl = "http://localhost:1337/explorer") {
  ...
  getDependencyJobs(jobId: string): string[] {
    ...
    if(decoded.circuit_type === ProvingJobCircuitType.WrapFinalSigHashProofBLS12381){
      ...
      if(found){
        const deps = [found.sighash_introspection, found.state_transition_reference].concat(this.getDependencyJobs(found.state_transition_reference))
        this.jobDependencies[jobId] = deps;
        return deps;
      }else{
        throw new Error("Could not find job "+jobId+" in sighash_proofs");
