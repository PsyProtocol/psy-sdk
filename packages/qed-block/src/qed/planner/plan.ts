...
function planJobs(
  ...
  let root_state_transition =
    blockStateTransitionInputWitnessJobId(checkpoint_id);

  let agg_jobs_for_inputs = Array.from(
    { length: num_input_witnesses },
    (v, i) => getBlockAggregateJobsGroupJobId(checkpoint_id, 1, i)
  );

  proof_store.write_next_jobs(agg_jobs_for_inputs, [
    notifyBlockCompleteJobId(checkpoint_id),
  ]);
  ...
  let state_part_1_common_id = getBlockAggregateJobsGroupJobId(
    checkpoint_id,
    op_agg_group_parts_common_id,
    0
  );
  let state_part_2_common_id = getBlockAggregateJobsGroupJobId(
    checkpoint_id,
    op_agg_group_parts_common_id,
    1
  );
  ...
  let register_users_agg_job_id = getBlockAggregateJobsGroupJobId(
    checkpoint_id,
    op_agg_group_part_1_id,
    0
  );
  let claim_deposits_agg_job_id = getBlockAggregateJobsGroupJobId(
    checkpoint_id,
    op_agg_group_part_1_id,
    1
  );
  let transfer_tokens_agg_job_id = getBlockAggregateJobsGroupJobId(
    checkpoint_id,
    op_agg_group_part_1_id,
    2
  );
  ...
  let add_withdrawals_agg_job_id = getBlockAggregateJobsGroupJobId(
    checkpoint_id,
    op_agg_group_part_2_id,
    0
  );
  let process_withdrawals_agg_job_id = getBlockAggregateJobsGroupJobId(
    checkpoint_id,
    op_agg_group_part_2_id,
    1
  );
  let add_deposits_agg_job_id = getBlockAggregateJobsGroupJobId(
    checkpoint_id,
    op_agg_group_part_2_id,
    2
  );
