...
const ExportPrivateKeyForm: React.FC<IExportPrivateKeyFormProps> = ({
  onComplete,
  className,
  privateKey,
}) => {
...
const ExportPrivateKeyModal: TAddressModalComponent = ({
  onCancel,
  onComplete,
}) => {
  ...
  useEffect(() => {
    if(currentWallet?.wallet.signer.getPrivateKeyHex && currentWallet.wallet.signer.getAbilities().includes("export-private-key-hex")){
    (async () => {
