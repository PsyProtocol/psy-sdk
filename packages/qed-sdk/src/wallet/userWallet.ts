...
class CityUserWallet implements ICityUserWallet {
  ...
  async getClaimDepositMessageHash(txidOrDepositId: string | number): Promise<{hash: string, deposit: ICityL1Deposit}> {
    const deposit = await (typeof txidOrDepositId === 'number'?this.rpc.getDepositById(MAX_CHECKPOINT_ID, 0):this.rpc.getDepositByTxid(txidOrDepositId));
  ...
  async claimDeposit(txidOrDepositId: string, signature: string, prover: ICitySecp256K1SignatureProver): Promise<void> {
