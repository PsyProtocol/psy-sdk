...
import { CityHash, ICityUserState, CityMerkleProof, ICityL1Deposit, Hash256, ICityL2BlockState, Hash160, ICityL1Withdrawal, QProvingJobDataIDSerializedWrapped, TProofValueStoreKV, ISimpleKVPair, ICityRegisterUserRPCRequest, ICityAddWithdrawalRPCRequest, ICityClaimDepositRPCRequest, ICityTokenTransferRPCRequest, SCFelt } from "./baseTypes";
...
class CityRPCProviderWithCache implements ICityRPCProvider {
  ...
  async getFromCacheOrFallback<T>(action: ()=>Promise<T>, isEmpty: (result: T)=>boolean, method: string, checkpointId: SCFelt, args?: (string | number)[]|(string | number)): Promise<T> {
  ...
  getUserById(checkpoint_id: number, user_id: number): Promise<ICityUserState> {
    ...
    return this.getFromCacheOrFallback<ICityUserState>(()=>this.base.getUserById(checkpoint_id, user_id), (result)=>result.public_key===ZERO_HASH, "getUserById", checkpoint_id, user_id);
  }
  ...
  getDepositById(checkpoint_id: number, deposit_id: number): Promise<ICityL1Deposit> {
    return this.getFromCacheOrFallback<ICityL1Deposit>(()=>this.base.getDepositById(checkpoint_id, deposit_id), (result)=>result.txid===ZERO_HASH, "getDepositById", checkpoint_id, deposit_id);  
  }
  async getDepositsById(checkpoint_id: number, deposit_ids: number[]): Promise<ICityL1Deposit[]> {
    ...
    const keys = deposit_ids.map((deposit_id, i)=>{
      ...
      return {index: i, key, has_cached, checkpoint_id, deposit_id, missingIndex: has_cached?-1:(missingIndex++)};
    });
  ...
  getWithdrawalById(checkpoint_id: number, withdrawal_id: number): Promise<ICityL1Withdrawal> {
    return this.getFromCacheOrFallback<ICityL1Withdrawal>(()=>this.base.getWithdrawalById(checkpoint_id, withdrawal_id), (result)=>(result.value+"")==="0", "getWithdrawalById", checkpoint_id, withdrawal_id);
  }
  async getWithdrawalsById(checkpoint_id: number, withdrawal_ids: number[]): Promise<ICityL1Withdrawal[]> {
    ...
    const keys = withdrawal_ids.map((withdrawal_id, i)=>{
      ...
      return {index: i, key, has_cached, checkpoint_id, withdrawal_id, missingIndex: has_cached?-1:(missingIndex++)};
    });
